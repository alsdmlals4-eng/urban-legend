...
10. PC 16:9 화면에서 버튼 순서, 마우스 클릭 흐름, 정보 위계가 자연스러운지 확인한다.
11. 기존 저승역/빨간 우산 시작, 이어하기, DB 진입, 저장 초기화 흐름이 유지되는지 확인한다.

## MVP-022 조사 화면 UI / 단서·힌트 추적 확인 항목

1. PC 16:9 화면에서 조사 화면이 사건 상태, 조사 포인트, 결과 로그, 단서 추적, 힌트 추적, 회수/안정화 안내로 구분되는지 확인한다.
2. 조사 포인트 클릭 시 기존 단서 수집, 힌트 기록, 조건 잠김 문구가 유지되는지 확인한다.
3. 단서 추적 영역에서 수집/미수집 상태가 구분되고, 수집된 단서 설명이 읽히는지 확인한다.
4. 힌트 추적 영역에서 확인/미확인 상태가 단서 수집률과 분리되어 보이는지 확인한다.
5. 조사 방법 결과가 `방법 → 성공/실패 → 새 정보 → 상태 변화 → 요원 반응 → 다음 행동` 순서로 읽히는지 확인한다.
6. 조사 방법 실행 후 요원 신뢰도 변화와 요원 이벤트가 기존처럼 표시되는지 확인한다.
7. `회수/안정화 시도` 버튼이 단서 조건에 따라 비활성/활성 상태를 유지하는지 확인한다.
8. 강제 회수전 조건이 발생했을 때 회수/안정화 안내와 버튼 문구가 흐름을 방해하지 않는지 확인한다.
9. 저승역과 빨간 우산 사건에서 조사 포인트, 조건 잠김, 미니게임 진입, 회수 페이즈 진입이 기존처럼 동작하는지 확인한다.

## 현재 남은 검증 위험

- 이 문서는 수동 확인 순서다. 실제 Godot 실행 결과는 사용자가 로컬에서 확인해야 한다.
- MVP-021은 UI 정보 구조 개선이므로, 기능 성공 여부와 별개로 실제 화면 가독성 확인이 필요하다.
- MVP-022는 조사 화면 정보 위계 개선이므로, 단서/힌트 추적과 조사 방법 결과 요약의 실제 가독성 확인이 필요하다.
- PC 창 모드/전체화면에서 정보 위계와 버튼 배치는 로컬 실행 화면에서 최종 판단한다.
