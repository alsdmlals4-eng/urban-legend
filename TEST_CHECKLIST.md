...
3. 조사 포인트를 클릭했을 때 새 단서가 결과 로그와 단서 목록에 반영되는지 확인한다.
4. 사건 상태와 단서 목록에서 수집/미수집 상태, 수집 단서 수/전체 단서 수가 보이는지 확인한다.
5. 힌트 목록이 단서 목록과 분리되어 표시되는지 확인한다.
6. 조사 방법 선택 결과가 `방법 → 성공/실패 → 새 정보 → 상태 변화 → 요원 반응 → 다음 행동` 순서로 읽히는지 확인한다.
7. 판정식과 상세 상태 변화가 결과 요약을 방해하지 않는 위치에 표시되는지 확인한다.
8. 회수/안정화 가능 여부가 현재 세계관 표현에 맞게 안내되는지 확인한다.
9. 저승역과 빨간 우산 사건 모두에서 조사 화면이 정상 동작하는지 확인한다.
10. PC 16:9 창 모드/전체화면에서 결과 로그와 단서/힌트 목록이 읽기 쉬운지 확인한다.
11. 기존 회수/해결 페이즈 진입 흐름이 유지되는지 확인한다.

## 현재 남은 검증 위험

- 이 문서는 수동 확인 순서다. 실제 Godot 실행 결과는 사용자가 로컬에서 확인해야 한다.
- MVP-022는 조사 화면 정보 구조 개선이므로, 기능 성공 여부와 별개로 실제 화면 가독성 확인이 필요하다.
- PC 창 모드/전체화면에서 조사 로그와 단서/힌트 영역 배치는 로컬 실행 화면에서 최종 판단한다.
