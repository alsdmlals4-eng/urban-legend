...
3. 저승역에서 플랫폼 스피커 확인 후 폐주파수 동기화에 진입해 축소 원이 판정 원과 겹칠 때 `Space/Enter`를 눌러 5박자 중 3회 성공하는지 확인한다.
4. 저승역에서 3박자 이상 놓치면 실패가 기록되지만 조사 복귀 버튼이 유지되는지 확인한다.
5. 빨간 우산에서 우산·표지판 단서를 얻은 뒤 빗소리 동기화에 진입해 방향키로 12초 동안 빗방울을 피하면 성공하는지 확인한다.
