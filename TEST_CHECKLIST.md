...
3. 저승역에서 플랫폼 스피커 확인 후 폐주파수 동기화에 진입해 `낮은 잡음 → 이름 파형 → 종착음` 순서의 성공과 오류 2회 실패를 각각 확인한다.
4. 빨간 우산에서 우산·표지판 단서를 얻은 뒤 빗소리 동기화에 진입해 `첫 빗방울 → 두 번째 메아리 → 세 번째 빈 프레임` 순서의 성공과 오류 2회 실패를 각각 확인한다.
5. 두 현장 판정 모두 실패 뒤 조사로 복귀할 수 있고 회수 흐름이 막히지 않는지 확인한다.
6. 성공/실패 후 위험도, 이해도, 정신력, 괴이 안정도와 회수 기준 변화가 판정 결과와 회수 근거에 표시되는지 확인한다.
7. 사건 완료 후 보고서와 기록국 DB의 미니게임 기록에 성공/실패, 결과 문구, 상태 변화 요약이 남는지 확인한다.
8. 저장 후 이어하기에서 현재 장면, 편성/대표 요원, 단서, 판정 결과, 회수 결과, 보고서, 장비/기록물, 신뢰도가 유지되는지 확인한다.
9. 저승역 완료 후 빨간 우산을 완료하고 DB에서 두 사건 보고서가 서로 구분되는지 확인한다.
10. README의 Steam 데모 후보 범위, 예상 시간, 조작, 저장 지원, 알려진 문제와 소개문이 현재 구현과 일치하는지 확인한다.
