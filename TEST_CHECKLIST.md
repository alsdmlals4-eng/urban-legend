...
5. 메인 메뉴가 모바일 세로 화면에서 스크롤되며 주요 행동 → 저장/요원 상태 → 개발/테스트 순서로 읽히는지 확인한다.
6. 요원 2명 미만일 때 시작 불가 이유가 명확히 표시되는지 확인한다.
7. 요원 2~3명을 선택하면 시작 가능 상태가 명확히 표시되는지 확인한다.
8. 준비 화면에서 현재 사건, 사건 선택, 요원 요약, 장비, 기록물, 로그, 조사 시작 영역이 카드 단위로 구분되는지 확인한다.
9. 장비 장착/해제와 사건 선택이 기존처럼 동작하는지 확인한다.
10. 조사 시작 가능/불가능 이유가 준비 화면에서 보이는지 확인한다.
11. 모바일 세로 화면 기준으로 스크롤 순서와 버튼 간격이 자연스러운지 확인한다.
12. 기존 저승역/빨간 우산 시작, 이어하기, DB 진입, 저장 초기화 흐름이 유지되는지 확인한다.
