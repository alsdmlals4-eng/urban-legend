...
5. 세 조사 포인트에서 단서 3개를 수집하고 수집률이 100%로 갱신되는지 확인한다.
6. 단서 2개 전에는 `CCTV가 끊긴 사거리: 빗소리 동기화`가 잠기고, 2개 후에는 열리는지 확인한다.
7. 빗소리 동기화 성공/실패 뒤 위험도, 이해도, 정신력, 회수 페이즈의 기준 또는 안정도 안내가 달라지는지 확인한다.
8. 회수 페이즈 제목이 빨간 우산 사건명으로 표시되고, 수집 단서와 동기화 결과가 자동 발동 안내에 나타나는지 확인한다.
9. 회수 후 결과 화면에 두 번째 사건의 피해자 결과와 연구 기록이 표시되는지 확인한다.
10. 기록국 DB의 `완료 사건 기록`에서 저승역과 빨간 우산 보고서가 별도 항목으로 남고, 저장 후 이어하기에도 유지되는지 확인한다.
...
- 기록국 DB UI는 MVP-023 전까지 완성형 검색/필터보다 목록 선택 + 상세 표시를 우선한다.
- 두 사건을 각각 완료한 뒤 메인 메뉴, 준비, 조사, 회수, 결과, DB 화면의 긴 문장이 모바일 세로 화면에서 읽히는지 MVP-021~028 동안 반복 확인한다.

## MVP-020 문서 기준 확인

1. `MVP_ROADMAP.md`가 `MVP-020 / Ver 2.0`과 GitHub Issue #27을 기준으로 표시하는지 확인한다.
2. 다음 MVP가 세 번째 사건이 아니라 메인/준비 UI 개선인 MVP-021로 정리되어 있는지 확인한다.
3. `docs/MVP_STATUS_AUDIT.md`에서 MVP-015~019의 유지·축소·인계 기준을 확인한다.
4. HTML 대시보드가 필요할 때는 GitHub 소스가 아닌 `C:/Users/user/Downloads/urban-legend-database-dashboard.html`을 연다.
