...
func _add_result_panel(parent: Control) -> void:
	...
	if reward.is_empty():
		content.add_child(_make_label("해금 능력명: 없음"))
		content.add_child(_make_label("해금 능력 설명: 아직 연구 보상이 없습니다."))
		content.add_child(_make_label("다음 사건 영향: 없음"))
		return

	content.add_child(_make_label("해금 능력명: %s" % reward.get("ability_name", "")))
	content.add_child(_make_label("해금 능력 설명: %s" % reward.get("ability_description", "")))
	content.add_child(_make_label("다음 사건 영향 placeholder: %s" % reward.get("next_episode_effect", "")))
