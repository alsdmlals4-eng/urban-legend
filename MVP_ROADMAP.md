...
- 확률은 중간 이해도에서 이미 관측한 회수 패턴의 행동명을 읽는 정보 해석에만 사용한다.
- 회수의 승리는 HP 0이 아니라 패턴 대응으로 `포획 창`을 여는 것이다.
- 기존 UX-PD-001 2B·2C와 MVP-044~046은 삭제하지 않고 PoC 뒤 재매핑한다.
- 전체 원문 대화·신규 플레이어 행동·사전 지표는 사용자의 결정에 따라 `POC_BUILD_READY` 차단 조건이 아니다.
- 플레이 증거 없이 `POC_PASSED`는 선언하지 않는다.
- 다음 마일스톤은 별도 사용자 승인 없이 시작하지 않는다.
...
> 실행 추적: Issue #56 | 구현 PR: PR #57 | 상태: `POC_BUILD_READY`

### 목표

> 플레이어가 조사에서 직접 만든 규칙 가설이 회수의 전조 정보가 되고, 준비한 대응으로 포획 창을 여는 순간을 독립 실행 가능한 슬라이스로 구현한다.
...
- 1280×720·1920×1080에서 핵심 UI와 Footer가 viewport 안에 유지된다.
- Esc는 상태를 되감지 않고 이전 단계를 읽기 전용으로 검토한다.
- PoC는 기존 캠페인 save를 생성하지 않는다.
...
| 2 | Python 데이터 계약 Green | PASS |
| 3 | Godot 로더 부재 Red | 확인 |
| 4 | 로더·로그 집중 테스트 | run #16 PASS |
| 5 | 상태 타입 경고 수정 | 반영 |
| 6 | import 조기 종료 회귀 | run #66 실패 원인 확인 |
| 7 | Godot `--import` 수정 | run #68 PASS |
| 8 | 문서 계약 | run #195 PASS |
| 9 | Python 데이터·정적 계약 | run #69 PASS |
| 10 | Godot 4.7.1 import | run #69 PASS |
| 11 | 집중 Godot 테스트 | run #69, 4/4 PASS |
| 12 | 전체 Godot 회귀 | run #69, 43/43 PASS |
| 13 | 해상도·Esc·포커스·저장 계약 | run #69 PASS |
| 14 | 보호 경로·review thread·mergeability | PASS |
...
`POC_BUILD_READY`는 구현·자동 계약·회귀·기계적 UI 검증이 통과했다는 뜻이다. 플레이테스트 통과 또는 제작 확대를 의미하지 않는다.
...
- main/nightly: Ubuntu·Windows × Python 3.11·3.12·3.13, Godot 전체 회귀는 Ubuntu 1회
