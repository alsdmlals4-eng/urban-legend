...
def main() -> int:
    ...
    discipline_skills = [entry for entry in skills.get("skills", []) if entry.get("layer") == "discipline" and entry.get("status") == "ACTIVE"]
    foundation_skills = [entry for entry in skills.get("skills", []) if entry.get("layer") == "foundation" and entry.get("status") == "ACTIVE"]
    if len(discipline_skills) != 11 or len(skills.get("discipline_entrypoints", {})) != 11: failures.append("exactly 11 active discipline skills and entrypoints are required")
    if len(foundation_skills) != 8: failures.append("exactly 8 selected active foundation skills are required")
    ...
    if failures:
        ...
        return 1
    print("Urban Legend operating-system contract: PASS (12 documents, 11 discipline skills, 8 foundation skills, 3 registries)")
    return 0
