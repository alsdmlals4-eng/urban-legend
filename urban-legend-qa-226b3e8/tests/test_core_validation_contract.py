...
class CoreValidationContractTests(unittest.TestCase):
    ...
    def test_manual_promotion_is_persistent_and_data_driven(self) -> None:
        game_state = (ROOT / "scripts/core/game_state.gd").read_text(encoding="utf-8")
        battle = (ROOT / "scripts/scenes/battle_scene.gd").read_text(encoding="utf-8")
        result = (ROOT / "scripts/scenes/result_scene.gd").read_text(encoding="utf-8")
        database = (ROOT / "scripts/ui/database_view.gd").read_text(encoding="utf-8")
        self.assertIn('var anomaly_manual_records: Dictionary = {}', game_state)
        self.assertIn('"anomaly_manual_records": get_anomaly_manual_records()', game_state)
        self.assertIn('save_data.get("anomaly_manual_records", {})', game_state)
        self.assertIn('_make_manual_decision_context', battle)
        self.assertIn('GameState.get_current_anomaly_manual_record()', result)
        self.assertIn('_show_anomaly_manual_records', database)
        self.assertNotIn('AfterlifeManualCatalog = preload', database)
