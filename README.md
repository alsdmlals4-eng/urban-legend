...
현재 버전은 `Ver 3.4`이며, MVP 번호를 소수점으로 반영합니다.

`MVP-034`는 사건 준비에 외부 접점을 추가하고, 소문시장·마도회·퇴마사 계열 관계와 잔향 파편 경제를 연결합니다. 고유 보상 ID로 중복 지급을 막고, 시장에서 영구 장비와 사건 반입 소모품을 구매합니다. 저장 스키마는 `mvp-034`입니다.
