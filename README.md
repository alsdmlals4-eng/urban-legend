...
현재 버전은 `Ver 1.6`이며, 이후에는 `Ver 1.7`, `Ver 1.8`처럼 갱신합니다.
...
- `agents`: 임무 투입 가능한 요원 id, 이름, 성향, 역할, 설명
...
이번 범위는 요원 데이터, 요원 2~3명 편성, 선택 요원 저장/불러오기, 3성향 기반 대화 반응까지만 포함합니다. 조사 방법 `파괴 / 관찰 / 분석`, 미니게임 보조, 회수 페이즈 지원, 신뢰도 변화는 다음 MVP 범위입니다.
...
17. 단서 2개 이상 수집 후 `해결 시도` 버튼이 활성화되는지 확인합니다.
18. 전투/회수/결과 화면까지 기존 흐름이 이어지는지 확인합니다.
19. 메인 메뉴로 돌아가 `이어하기`가 미니게임 결과, 선택 요원, 마지막 저장 씬을 복원하는지 확인합니다.

## 체크리스트

- [ ] Godot 4.7 stable에서 프로젝트가 열린다.
- [ ] 메인 메뉴의 버전 표기가 `Ver 1.6`으로 표시된다.
