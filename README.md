...
현재 버전은 `Ver 3.5`이며, MVP 번호를 소수점으로 반영합니다.
...
메인 조사 화면은 배경과 상황 설명을 우선하고, 필요한 로그 안내와 편성 요원의 짧은 얼굴 초상 반응을 하단 선택 도크에 함께 표시합니다. 조사 지점과 방식은 34:66 두 열에서 동시에 비교하며, 회수 화면은 대표 요원과 괴이의 대치 장면 위에 얇은 상태 HUD와 액션 카드만 겹칩니다. 상세 단서와 학습 기록은 필요할 때 여는 서랍으로 이동했고 저장 스키마는 계속 `mvp-035`입니다.
