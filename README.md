...
현재 버전은 `Ver 2.8`이며, MVP 번호를 소수점으로 반영합니다. 예를 들어 MVP-018은 `Ver 1.8`, MVP-020은 `Ver 2.0`, MVP-022는 `Ver 2.2`, MVP-028은 `Ver 2.8`입니다.
...
- [ ] 메인 메뉴의 버전 표기가 현재 기준 `Ver 2.8`로 표시된다.
...
`equip_frequency_filter`는 전투력 장비가 아니라 기록국식 조사 보조 도구입니다. 장착 상태로 주파수 계열 미니게임에 들어가면 “폐주파수 필터가 잡음을 분리합니다. 다음 타이밍은 세 번째 파형이 가장 안정적입니다.” 힌트를 1회 표시하고, `used_equipment_effects`로 같은 미니게임 중복 발동을 막습니다.
...
다음 작업은 MVP-021입니다. 새 저장 필드나 게임 규칙을 늘리지 않고, 시작 화면과 사건 준비 화면에서 선택한 사건, 편성 요원, 장착 도구, 저장 상태, 다음 행동을 더 명확하게 보여 주는 UI를 정리합니다.
