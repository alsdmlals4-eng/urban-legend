...
현재 버전은 `Ver 3.2`이며, MVP 번호를 소수점으로 반영합니다. 예를 들어 MVP-018은 `Ver 1.8`, MVP-022는 `Ver 2.2`, MVP-028은 `Ver 2.8`, MVP-030은 `Ver 3.0`, MVP-031은 `Ver 3.1`, MVP-032는 `Ver 3.2`입니다.

`MVP-032`는 요원을 `제압·분석·방호·치료·교감` 5능력과 사건별 체력·정신력으로 확장합니다. 편성은 사건 준비에서 2~3명을 고르고, 상세 프로필에서 고유 장비·기술·면모·백스토리를 확인합니다. 조사에서는 현재 선택에 필요한 능력만 표시하고, 회수/안정화에서는 다섯 능력과 방호·회복·괴이 반응을 실제 사건 상태에 연결합니다. 저장 스키마는 `mvp-032`입니다.
...
- [ ] 메인 메뉴의 버전 표기가 현재 기준 `Ver 3.2`로 표시된다.
