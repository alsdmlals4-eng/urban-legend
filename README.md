...
현재 버전은 `Ver 3.3`이며, MVP 번호를 소수점으로 반영합니다.

`MVP-033`은 대화와 조사를 연속 현장 노드로 통합하고, 선택 후 3~5회의 팀 대화를 거쳐 다음 선택으로 진행합니다. 회수는 괴이 전조·공정한 자동 예측·상황형 대응·요원 자동 보조·오대응 학습으로 진행하며 안정도는 0에서 100으로 상승합니다. 저장 스키마는 `mvp-033`입니다.
...
- [ ] 메인 메뉴의 버전 표기가 현재 기준 `Ver 3.3`으로 표시된다.
