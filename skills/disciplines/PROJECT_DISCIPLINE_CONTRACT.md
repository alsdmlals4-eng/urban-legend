...
4. `support_skills`는 Base 지원 Skill만 등록하며, 목록 전체가 아니라 현재 trigger와 단계에 맞는 것만 최대 3개 선택한다.
5. `related_disciplines`는 동시에 실행할 주 Skill 목록이 아니라 참조·검토·후속 인수인계 대상이다.
6. 실제 대상 코드·데이터·Scene·자산·문서·테스트를 확인한다.

Registry 행이나 이 공통 계약만 읽고 해당 Skill을 실행했다고 보고하지 않는다. 여러 분야가 연관돼도 최종 결정을 소유하는 주 프로젝트 Skill은 한 시점에 하나다.
...
전체 Skill 폴더 기본 로드, 여러 주 분야 Skill 동시 선택, 관련 분야를 Base 지원 Skill처럼 호출, 계획을 구현 완료로 표시, 보호 경로 무승인 변경, 미실행 검사 PASS, 기능 손실을 숨기는 삭제·통합, 코어를 암묵 변경하면 실패다.
