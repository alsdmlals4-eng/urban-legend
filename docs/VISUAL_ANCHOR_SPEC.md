...
Status: `PLANNING_COMPLETE / RUNTIME_IMPLEMENTED / M04_RECOVERY_AND_D_RUNTIME_VERIFIED / PRODUCT_REFERENCE_ASSET_PENDING / HUMAN_QA_NOT_RUN`
...
- root manifest에는 12개 `PROJECT_ASSET_APPROVED` entry가 있다: Bureau Archive main-menu background/emblem/wordmark, M01 Investigation/Recovery background, M01 B/C·D cutout, CASE-01 루메 매뉴얼 보조 초상, M04 Investigation/Recovery background, M04 B/C·D cutout. Bureau Archive 세 항목은 scenario art가 아닌 title-screen의 `MainMenuBackdrop`과 `WorldTitleLockup`만 소비하며, runtime evidence와 Human/accessibility/release gate를 분리한다.
