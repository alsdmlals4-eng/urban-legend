...
*화면형 구현 후보: 실제 사건의 정답·단서·런타임 화면이 아니라, 플레이어가 출처가
남은 키워드를 읽고 빈 규칙 문장을 작성하는 인게임 매뉴얼의 정보 구조를 정확한
문구로 보여 주는 사람용 시안이다. 하나의 기록철 프레임 안에서 왼쪽 INDEX, 중앙
추리문, 오른쪽 2열 후보와 기록관 루메 보조를 동시에 비교한다. 루메의 저승역
복장은 사건 맥락 전용이며, 다른 시나리오에서는 별도 후보·별도 승인 절차로
복장이 바뀐다. 후보의 색·크기·정렬은 정답이나 변형 여부를 알려주지 않으며,
이 화면은 `USER_APPROVED / BLUEPRINT_REFERENCE_ONLY`이며, 실제 제품 자산·런타임
화면·Human QA 승인과는 별도다.*
...
| 하단 · 후보 규칙과 아카 | 구조 상태와 현장 복귀 목적 | 후보 규칙을 저장하고 구출·회수에서 검증할 준비를 한다. | 아카가 숨은 답이나 가장 안전한 후보를 대신 고르지 않는다. |
...
아래 화면은 실제 게임 화면의 배치와 판단 순서를 설명하기 위한 **화면형 설명 시안**이다.
현재 런타임 스크린샷이 아니며, 특히 플레이어가 빈칸을 직접 채우는 입력은 승인된 설계이지만 아직 구현되지 않았다. 다만 각 화면의 입력·출력·판단 경계는 현재 프로젝트의 매뉴얼 계약을 그대로 따른다.
...
`[K-01]` 같은 표기는 **실제 사건 데이터의 새 키워드가 아니라 화면 형식 예시**다. 현재 M01에는 플레이어가 조합할 후보 풀 데이터가 아직 없으므로, 블루프린트가 새 사건의 정답이나 단서를 발명하지 않는다.
...
| --- | --- | --- |
| 조사 결과와 매뉴얼 열람 | 현재 기준선 구현됨 | 관측 기록과 현재 정보를 확인하는 출발 화면. |
| 빈칸 매뉴얼에 후보를 직접 배치 | `USER_APPROVED / NOT_IMPLEMENTED` | 화면형 설명 시안으로만 보여 준다. 실제 입력 UI가 있다는 뜻이 아니다. |
| 매뉴얼 작성 직후 정답·오답 자동 표시 | 의도적으로 금지 | 문서에서도 점수·체크 표시 대신 `후보 규칙`으로만 표시한다. |
| 구출과 회수에서의 필드 검증 | 공통 흐름 기준선 존재, Human QA 미실행 | 보호 절차와 `전조 → 가설 → 근거 → 대응`의 연결로 보여 준다. |
