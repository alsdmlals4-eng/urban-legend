...
7. `docs/AI_SKILL_ADOPTION_GUIDE.md` - 외부 스킬을 쓰거나 긴 작업 context를 줄이는 경우
8. `docs/CODEX_SHARED_WORK_RULES.md`
9. `docs/MVP_WORKFLOW_CHECKLIST.md`
10. `docs/BENCHMARKING_REFERENCE_GUIDE.md` - 기획 판단이나 MVP 범위 판단이 필요한 경우
11. 현재 GitHub Issue 본문
12. 현재 Codex Goal
13. `README.md`
14. 실제 수정 대상 JSON / GDScript / TSCN 파일
...
스킬 선택, 설치 전 권한 확인, 최소 라우팅, context compact는 `docs/AI_SKILL_ADOPTION_GUIDE.md`를 따른다.

- Superpowers, Serena, Harness와 외부 스킬은 선택형 보조 수단이다.
- 사용 가능한 스킬 중 현재 작업 trigger에 맞는 최소 조합만 사용한다.
- 사용자가 설치를 요청하지 않았다면 저장소 구조와 원칙만 반영한다.
- 플러그인이 없거나 실패하면 Issue/Goal/로컬 규칙과 기본 도구로 계속 진행한다.
- 실제 사용 여부와 대체 검증은 사실대로 보고한다.
- 최신 사용자 지시, `AGENTS.md`, 현재 Issue/Goal, 실제 파일 상태가 항상 우선한다.
...
MVP-028부터 HTML 대시보드는 기본 산출물이 아니다. 사용자 최신 지시가 명시할 때만 별도 범위로 다룬다.
...
- 필요한 경우 `docs/AI_SKILL_ADOPTION_GUIDE.md`에 따른 스킬 라우팅
- 현재 Issue 번호
- 작업 목표
- 실제 수정할 구현 범위
- 제외 범위
