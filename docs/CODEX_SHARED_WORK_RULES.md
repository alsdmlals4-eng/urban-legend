...
> 이 파일은 현재 `도시괴담 기록국` 프로젝트 전용 규칙이다. Base 원격 링크가 아니라 `docs/BASE_RULES_VERSION.md`와 이 저장소의 공용 로컬 사본을 먼저 읽은 뒤, 이 프로젝트의 Godot·폴더·세계관 규칙을 적용한다. 엔진·폴더·세계관에 종속되지 않는 원칙은 Base와 `docs/AI_SHARED_WORK_RULES.md`에 두며, 새 프로젝트에서는 공용 문서를 먼저 동기화한 뒤 프로젝트 전용 규칙을 별도로 작성한다.
...
8. 이 프로젝트 전용 `docs/CODEX_SHARED_WORK_RULES.md`
9. `README.md`
10. `PROJECT_BRIEF.md`
11. `DESIGN_INTENT.md`
12. `MVP_ROADMAP.md`
13. `TEST_CHECKLIST.md`
14. 현재 GitHub Issue 본문
15. 실제 수정 대상 JSON / GDScript / TSCN 파일

10~13번 문서가 없으면 부재 사실을 기록하고 추정하지 않는다. Base 원격 저장소는 동기화 또는 기준 차이 확인이 필요할 때만 추가로 확인한다. 실제 파일 구조를 확인한 뒤 수정한다.
