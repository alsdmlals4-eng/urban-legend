...
4. 검토할 Human QA checklist를 실행 전에 표시하고 evidence 폴더에 저장한다.
5. 격리 APPDATA로 Godot 에디터를 실행한다.
6. 사용자가 실제 화면·조작·저장 동작을 검토한다.
7. Godot 종료 후 항목별 판정을 입력한다.
8. 원본 불변성과 QA 산출물을 수집한다.
9. 저장 본문이나 절대 경로를 노출하지 않는 Human QA 결과 요약을 생성한다.
...
기존 `tools/qa/run_afterlife_canon_v2_human_qa.ps1`을 안전 경계와 저장 처리의 단일 권위 구현으로 유지한다. 새 패키지는 기존 runner를 복제하거나 재작성하지 않고 다음의 얇은 진입 계층만 추가한다.

- `START_HUMAN_QA.cmd`
  - 저장소 루트에서 더블클릭 가능한 Windows 진입점
  - `pwsh`가 있으면 PowerShell 7을 우선 사용하고, 없으면 Windows PowerShell 5.1 `powershell.exe`를 사용
  - UTF-8 콘솔 설정과 실행 정책을 해당 프로세스에만 적용
...
- `tools/qa/afterlife_canon_v2_human_qa_checklist.json`
  - 사람이 판정해야 할 항목과 허용 상태를 정의하는 권위 데이터
...
7. checklist를 콘솔에 표시하고 `<QA_ROOT>/evidence/HUMAN_QA_CHECKLIST.md` 생성
8. 기존 runner의 `Launch -WaitForExit` 실행
...
- dirty working tree는 경고하되 제품 파일을 수정하지 않으므로 기본적으로 진행 가능하다. 단, `START_HUMAN_QA.cmd`, `tools/qa/**`, `project.godot`이 변경된 경우에는 재확인을 요구한다.
- 사용자가 checklist 입력을 중단하면 나머지 항목은 `NOT_RUN`으로 기록하고 전체 결과는 `HUMAN_QA_INCOMPLETE`로 분류한다.
...
발견된 실행 파일은 `--version`으로 검증한다. Godot이 출력하는 공식 빌드 접미사는 허용하되 주 버전은 정확히 파싱한다.

- 기본 허용: 파싱된 버전이 `4.7.1`
- 다른 4.7.x: 경고 후 명시적 사용자 승인 필요
- 4.7 미만 또는 5.x 이상: 기본 차단
- 자동화 테스트에서는 `-AllowVersionMismatch`를 명시해야만 우회 가능
...
checklist는 코드에 흩어진 문자열이 아니라 `tools/qa/afterlife_canon_v2_human_qa_checklist.json`에 권위 데이터로 둔다.
...
- `.cmd` 진입점 존재와 PowerShell 7 우선·5.1 대체 호출
...
GitHub-hosted Windows에서 실제 Windows PowerShell 5.1 및 PowerShell 7 호환성을 검증한다.

테스트용 임시 저장과 fake Godot executable을 사용해 다음을 확인한다.

- 정상 Godot 탐색
- 다중 후보 우선순위
- 공식 빌드 접미사가 포함된 4.7.1 버전 허용
- 버전 불일치 차단
- 본편 저장 없음 차단
...
3. checklist는 Godot 실행 전에 표시되고 evidence 폴더에도 저장된다.
4. 원본 저장은 Prepare 전후와 Collect 시점에 SHA-256 불변이 검증된다.
5. Godot은 격리 APPDATA에서 실행된다.
6. 사용자가 Human checklist를 기록하기 전에는 Human QA가 PASS로 표시되지 않는다.
7. 결과 요약에는 저장 본문과 원본 절대 경로가 없다.
8. Windows PowerShell 5.1과 PowerShell 7 테스트가 통과한다.
9. 기존 runner의 공개 사용법과 저장 안전 계약이 깨지지 않는다.
10. full Godot regression과 Project Base Adapter가 통과한다.
11. PR은 검증 증거와 exact HEAD를 기록하며 별도 승인 전에는 `main`에 병합하지 않는다.
