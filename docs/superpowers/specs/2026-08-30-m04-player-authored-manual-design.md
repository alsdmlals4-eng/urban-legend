...
M04 빨간 우산의 플레이어가 조사에서 얻은 세 단서를 출처와 함께 읽고, 빈칸이 있는 두 개의 추리문에 후보 키워드를 직접 배치한 뒤, 이미 존재하는 구출·회수 흐름에서 자신의 판단을 검증할 수 있게 한다.
...
M04 가이드는 이미지 없는 텍스트 보조인 `기록관 아카`다. CASE-01에만 루메의 승인된 저승역 초상을 표시한다. 동일 workbench의 기술적 호환 노드 이름은 CASE-01 회귀를 위해 유지할 수 있지만, M04에서는 초상 노드를 숨기고 어떤 루메 텍스트도 노출하지 않는다.
...
- 기존 `minigame_rain_sync`, rescue gate, recovery pattern, 결과·보상·저장 버전의 의미는 바꾸지 않는다.

## 비목표

- M05~M12 사건, 새 괴이·단서·플래그·미니게임·밸런스·save schema를 만들지 않는다.
- 새 가이드 일러스트나 이미지를 생성·승격하지 않는다.
...
3. M04는 `기록관 아카`와 초상 숨김을, M01은 루메와 승인된 초상을 보존한다.
4. M04 baseline, M01 manual regression, 1280×720 및 1920×1080 scene test, package and full Godot regression, Python suite가 통과한다.
