...
> 상태: `WRITTEN_FOR_USER_REVIEW / DIRECTION_ALREADY_APPROVED / IMPLEMENTATION_NOT_STARTED`
> 날짜: 2026-08-08
...
Spec 승인 뒤 구현 계획은 다음 최소 slice로 분리한다.
...
이 문서는 승인된 방향을 구현 가능한 계약으로 세분화한 **written design spec**이다.
...
→ HiGodot 권위로 Godot mutation
→ GUT/CI
→ actual Human/UI QA
```

이 review gate를 통과하기 전에는 Godot Scene·Node·Resource·Project Settings의 persistent mutation을 시작하지 않는다.
