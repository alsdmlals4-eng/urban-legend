...
4. SKILL_REGISTRY trigger로 최소 Skill·Skill Mode 선택
5. 대상 ZIP/Issue/Goal의 범위 확인
6. IMP-00 사전 감사
7. 작은 end-to-end 구현 단위 확정
8. 구현 → 자동 검증 → 수동 화면 검증
9. 상태·로드맵·테스트 갱신
10. Skill 실행 결과 보고·main 통합
```

모든 과거 Goal·QA·벤치마크와 전체 Base Skill을 읽지 않는다. 현재 변경과 직접 연결된 파일만 `docs/DOCUMENTATION_MAP.md`와 `skills/SKILL_REGISTRY.json`에서 선택한다.
...
- [ ] Registry의 Base commit, 13개 Skill, legacy alias, 프로젝트 책임 경로가 일치한다.
- [ ] 미실행 항목을 통과로 쓰지 않는다.
...
플레이어 화면·입력·런타임 파일을 변경하지 않은 문서·거버넌스 PR은 수동 플레이 검증을 자동 PASS로 바꾸지 않고 `NOT_RUN — runtime scope unchanged`로 기록한다.
