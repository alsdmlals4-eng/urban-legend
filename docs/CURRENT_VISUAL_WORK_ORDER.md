...
Root `ASSET_MANIFEST.yml`가 current tracked product-asset authority이며, 현재 승인된 M01 Investigation/Recovery background·B/C cutout·D cutout·CASE-01 루메 매뉴얼 보조 초상 및 M04 Investigation/Recovery background와 M04 B/C·D cutout·CASE-04 루메 매뉴얼 보조 초상 (`CASE-04 Lume guide`)의 10개 entry를 가진다. CASE-04 루메 초상은 M04 `LumeGuidePanel/LumePortrait`의 scenario view model로만 소비하며, 저승역 복장을 재사용하지 않고 정답 정보를 전달하지 않는다.
