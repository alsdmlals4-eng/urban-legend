...
29. `M01_ENTRANCE_BACKGROUND_ADAPT_02_20260828` one-image re-entry candidate — `USER_AUTONOMOUSLY_AUTHORIZED_VISUAL_CANDIDATE / PRODUCT_ASSET_PROMOTION_PENDING`; restores the descending entry threshold, paired rails, and clock landmark missing from Adapt 01. Preserve `afterlife_entrance.png`; a separate actual-consumer comparison and promotion task is required.
30. `M04_RECOVERY_BACKGROUND_ADAPT_02_20260828` one-image re-entry candidate — `USER_AUTONOMOUSLY_AUTHORIZED_VISUAL_CANDIDATE / PRODUCT_ASSET_PROMOTION_PENDING`; uses an empty closed-shop alley with an upper-middle anomaly zone and low-detail ActionDock zone, correcting the photographic surface concern from Adapt 01. Preserve `red_recovery.png` and all separate overlays; a separate composited runtime comparison and promotion task is required.
