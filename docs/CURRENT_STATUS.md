...
## Base·Skill 운영체계 동기화 — 게임 구현과 별도

| 항목 | 상태 | 설명 |
|---|---|---|
| Base 기준 | 검토 중 | `alsdmlals4-eng/Base@ee265576da7f67d3278f8099dd97d4e714ef0651`, Registry blob `2291bce0...` |
| 비파괴 동기화 | Draft PR #46 | 기존 `docs/`, GDD, Roadmap, Test, 게임 파일 경로를 유지하고 운영 모델·실행 Skill·검증을 추가 |
| Base Skill | 검증 완료 | 활성 13개 ID·trigger·비사용 조건·review trigger를 고정 원본과 대조 |
| 프로젝트 분야 Skill | 검증 완료 | 실제 `SKILL.md` 실행 패키지 10개, Registry↔패키지 1:1 |
| Skill 통합 | 검증 완료 | `urban-legend-integration-review`를 Base 통합검수·운영체계 `verify`로 통합, Alias 유지 |
| 자동 라우팅 | 검증 완료 | 프로젝트 trigger 완전 중복 없음, 대표 routing example로 주 Skill 최대 1개·지원 Skill 최대 3개 확인 |
| PR #41~#43 | 병합 차단 권고 | 승인 없는 대규모 이동, 루트 규칙 축약, 구형 Foundation Skill 활성화가 최신 Base와 이주 계약에 충돌 |
| 게임 코드·데이터·Scene·자산 | 변경 없음 | Base·Skill 동기화 범위에서 제외 |
| Godot·수동 플레이 QA | `NOT_RUN` | 플레이어 화면과 런타임 파일을 변경하지 않음 |

Base 동기화 PR이 존재한다는 사실은 게임 기능 구현 완료나 프로젝트 구조 이주 완료를 뜻하지 않는다. 병합 전 다음을 모두 확인한다.

```text
python -m unittest tests/test_base_operating_sync.py tests/test_skill_package_integrity.py
→ PR changed-file 감사
→ 현재 head GitHub Actions 성공
→ 보호 게임 경로·삭제·rename 없음
```

상세 감사: `docs/qa/BASE_SYNC_AUDIT_2026-07-21.md`.
...
3. Registry trigger로 주 프로젝트 분야 Skill 최대 1개와 필요한 Base Skill을 선택하고 실제 `SKILL.md`를 읽는다.
4. 분야별 기획서에서 플레이어 가치·금지 방향·수용 기준을 확인한다.
5. 구현되지 않은 계획을 `구현 확인`으로 문서에 쓰지 않는다.
6. 작은 end-to-end 단위로 구현·테스트·수동 QA한다.
7. 완료 후 이 문서, `MVP_ROADMAP.md`, `TEST_CHECKLIST.md`, 분야별 기획서를 갱신한다.
...
- 프로젝트 분야 Skill: `skills/disciplines/`
