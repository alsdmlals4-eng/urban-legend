...
플레이어는 고정 주인공 **권나래**의 1년 일정·역량·신념·관계를 육성하고, 텍스트 노벨형 조사에서 관측 가능한 단서와 키워드로 **괴이 매뉴얼**의 후보 규칙을 구성한 뒤, 피해자 구출 미니게임과 턴제 회수 전투에서 그 이해를 적용하고, 성공과 실패를 연구·장비·동료 협업·다음 연도의 기록으로 축적한다.

## 장르 정의

> **주인공 육성 시뮬레이션 + 텍스트 노벨 + 규칙 추리 + 피해자 구출 미니게임 + 턴제 회수 전투**

텍스트 노벨은 일상·관계·기관 업무·사건 조사·미니게임 전후·회수 전투·분기 및 연도 결산을 연결하는 표현 문법이다. 피해자 구출 미니게임과 턴제 회수 전투를 제거하는 선언이 아니다.
...
> 내가 육성하고 준비한 만큼 더 많은 위험을 감당할 수 있고, 내가 조사한 만큼 피해자를 더 안전하게 구출하고 회수 전투에서 괴이를 더 정확히 읽을 수 있으며, 성공과 실패를 기록한 만큼 다음 출현과 다음 연도에서 더 나은 선택을 할 수 있다.
...
9. 성공과 실패를 공식 규칙·위험 사례·연구·관계·분기 정산·연도 결산으로 되돌린다.
