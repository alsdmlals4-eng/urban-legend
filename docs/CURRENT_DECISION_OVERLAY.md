# 2026-09-20 승인 제품 continuation — 통합·회귀 교정

결정 ID: `D-2026-09-20-DAILY-CASE-INTEGRATION`. 사용자의 “좋아 작업 계속 진행해”에 따라 병합 완료된 #362/#363 다음의 기존 W01–W12 제품 계약을 이어간다. 별도 새 게임 설계 승인이 아니라 원래 44개 제품 커밋과 최신 운영 정본의 재결합, 실제 consumer 검증과 회귀 교정이다.

- 현재 제품은 `DAILY_CASE` / 루메 / 원본 관측→후보 해석→실제 현장 행동이다. 아래 과거 10일/아카는 compatibility/history이며 복원하지 않는다.
- 최신 main에서 만든 current-task integration branch만 수정한다. 다른 열린 PR head·원래 dirty checkout·승인 자산·저장 schema/ID·Base pin·설치와 전역 설정은 보호한다.
- 새 해금 규칙은 별도 결정이다. 폐주파수 자연 발견의 달력 의존이 끊긴 것은 P1이며, fixture risk 주입이나 테스트 삭제로 완료를 주장하지 않는다. 사용자에게 해금 대안을 질문했고 결정 대기다.
- 사용자 최종 UX/재미/아트/출시를 자동 검사나 이번 정상 입력으로 대체하지 않는다. 통합 상태·검사·남은 작업은 `CURRENT_HANDOFF.md`와 기존 `2026-09-14-remaining-implementation-contract.md`의 실행 기록을 따른다.
...
## 2026-09-16 장면/지문 분리 및 월간 일지 누적 갱신

- 최신 사용자 지시로 조사 장면과 읽기 UI를 서로 겹치지 않는 영역으로 분리한다. 도입·후속 대사와 긴 결과는 짧은 분량으로 순차 표시하고 버튼/키보드로 넘긴다. 기존 사건 원문·판정·저장 의미는 유지한다.
- 기존 월간 작업일지 PDF 및 evidence index에 날짜별 요약과 새 근거를 누적한다. 새 작업마다 별도 일지/새 PDF 파일을 만들지 않는다. 아래의 기존 발행본 덮어쓰기 금지는 이 명시 지시로 대체하며, 이전 출력 hash·갱신 사유와 기존 작업 기록은 보존한다.
- 이번 작업을 검증 후 GitHub 작업 브랜치에 동기화한다. 다른 진행 PR이나 사용자 import/UID는 변경하지 않는다.

## 2026-09-14 회수 시전 컷인 추가 지시

사용자 제공 `Skill_1714356970996.gif`는 회수 행동의 시전 컷인 연출 참고다. 직원의 시전 컷인→현장의 억제/보호 효과→결과/시계 변화가 실제 행동과 연결되어야 한다. 기존 대표 이미지 0.9초 표시만으로 이 요구 완료를 선언하지 않는다. 비교 게임의 캐릭터/효과/픽셀 그림체는 복제하거나 제품 자산으로 등록하지 않는다. 조사 UI 작업과 병행할 W07 요구이며 실제 자산·motion·소비처·입력/시계 정지·효과 감소/스킵·중복 판정 방지 검증이 필요하다.

## 2026-09-14 조사 UI · 서사형 텍스트 RPG 전환 지시

사용자는 조사창과 조사 화면 전체가 딱딱하다고 지적하고 활협전·화산의 딸을 참고한 텍스트 RPG 표현으로 변경하도록 지시했다. 다음 우선 작업은 W06 조사 표현 재구성이다. 기존 패널 배치의 유지 자체를 승인 목표로 간주하지 않는다.

- 사용자 승인 방향: 현장·인물·서술·행동 선택이 중심인 조사 경험. 기존 괴이기록국의 SOFT_ANIME_NOIR와 기록/매뉴얼 정체성은 유지한다.
- 보호: 단서 원문·획득 조건·기존 선택 ID·판정/위험·저장·매뉴얼 초안과 회수 연결. 일정/육성 시스템이나 비교 게임의 자산을 이식하지 않는다.
- 구현 전 화면/상태별 수정 계획을 제시하고, 기존 연속작업 승인 안에서 진행한다. 세부 배치와 최종 시각 만족은 아직 검증/승인된 결과가 아니다.
- 조사 비교·대안·구현 진입점은 CURRENT_HANDOFF의 같은 날짜 항목을 따른다. 이 지시는 게임 전체 잔여 명세를 취소하지 않는다.

## 2026-09-14 이미지 분리·월별 증빙집 사용자 지시

- 향후 이미지 생성은 크로마키 배경 생성→배경 제거→실제 alpha/가장자리 잔색 검수 순서를 적용한다. 배경 분리가 필요한 캐릭터·오브젝트·UI 자산이 대상이며 완성 배경 그림 자체를 투명하게 지우지 않는다. 이번 지시로 기존 승인 이미지를 일괄 교체하지 않는다.
- 블루프린트와 별도로 프로젝트별 월간 「AI 활용 작업일지·증빙집」 PDF를 만든다. 원본 코드/기록의 파생 보고서이며 새 기획 정본이 아니다.
- 제출본 지정 경로 `C:/Users/user/Documents/증빙서류/9월 증빙서류`는 사용자 명시 외부 저장 예외다. 생성 도구/검증/공개 가능한 evidence index는 프로젝트 안에 둔다.
- 실제 작업일(확인 범위), Git 기록시각, 기록 작성일, 캡처 시각, 발행일을 구분한다. 원본 미확보는 추정하지 않고 표시한다. 최초 제출 여부는 확인되지 않았으며 발행본을 조용히 덮어쓰지 않는다.
- 사용자 제공 메일/협약서 요약은 참고자료다. 본 작업에서 원문 확인·법적 검토·회신·서명·비용 인정 확인을 수행했다고 적지 않는다. 개인정보·영수증 원본은 공개 저장소에 넣지 않는다.
