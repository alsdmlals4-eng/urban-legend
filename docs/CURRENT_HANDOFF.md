...
동일 설계 §10에 일상 상태 전이·실제 키워드 소비자·시계 진행 차이·텍스트 와이어프레임을 추가했다. 다음 제품 결정은 현장 시간 진행과 매뉴얼 열람 정지 정책이다. 현재 턴 기반 시계를 실시간 구현 완료로 보고하지 않는다. 선택 기억 저장과 후속 대사 반응, 작성 노트와 authored 규칙 결과 텍스트를 구분한다.
