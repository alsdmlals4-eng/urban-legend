...
> MVP-043 + CORE-VALIDATION-001 반영: 권나래 고정 주인공과 초기 5인 구조를 유지하며, 저승역 안정화는 전조·가설·근거·대응을 분리한다. 현장 성공과 공식 매뉴얼 승격을 별도 판정하고 공식 규칙·검증 대기 후보·위험 사례를 결과·DB·저장에 연결한다.
...
|---|---|
| 문서 버전 | v1.1 |
| 문서 상태 | MVP-043 + CORE-VALIDATION-001 단계형 추론·매뉴얼 승격·저장·결과·DB 통합 |
| 기준 게임 버전 | Ver 4.2 |
...
| 핵심 한 문장 | 인간의 마음과 기억에서 되살아나는 괴이의 규칙을 조사하고, 현재 출현을 안정화해 다음 피해를 막을 매뉴얼을 기록한다. | 구현 확인 |
| 플레이어 역할 | 주인공 요원 한 명과 최대 두 명의 서포트를 편성하는 현장 판단 담당 | 구현 확인 |
| 핵심 원칙 | 괴이는 처치 대상이 아니라, 규칙을 밝혀 안정화하고 대응을 기록할 현상이다. | 확정 설계 |
...
- HQ에서 선택형 요원 대화를 읽되, 캠페인 일정과 핵심 추리를 소모하지 않는다.
...
| HQ 일상 | 선택 대화가 일정과 필수 추리를 침범하지 않는가 | 즉시 결과·요원 반응·DB 기록만 남기고 같은 준비 화면으로 돌아온다 | 반일·위험·의뢰가 바뀌거나 필수 정답을 알려 준다 | 구현 확인 |

## 3. 세계관과 표현

괴이 기록국은 인간의 마음과 기억에서 되살아나는 괴이의 반복 규칙을 조사하고, 현재 출현을 안정화하며, 재출현 때 더 적은 피해로 대응할 괴이 매뉴얼을 남기는 기관이다. 괴이를 영구 제거하는 대신 검증된 대응과 잔향을 기록한다.
...
신규 캠페인
→ 현재 반일 계획
→ (선택) HQ 일상 에피소드 확인 — 일정 비소모·즉시 결과·DB 기록
→ 주인공 반일 일정과 서포트 0~2명 편성
...
| HQ 일상 단위 | 준비 화면의 선택 대화 1장 | 요원 대화와 두 선택지 중 하나를 읽는다 | 즉시 결과·요원 반응·DB 기록·최초 이해도 보상 | 반일·위험·조사 편성·의뢰·필수 단서·회수 패턴을 바꾸지 않는다 |

### MVP-043 + CORE-VALIDATION-001 주인공·서포트와 저승역 판단 규칙

- 저장 호환: 권나래를 모든 저장의 `selected_agent_ids[0]`로 승격하고 나머지 최대 두 명을 서포트로 해석한다. 현재 반일의 기존 주인공 일정은 권나래에게 복사하되 원본은 보존한다. 새 저장 필드나 저장 버전은 없다.
- 일정: 반일 일정은 주인공에게만 배정·소비한다. 서포트는 독립 일정표를 갖지 않는다.
- 저승역 튜토리얼: 권나래 주인공, 오현·강이준 서포트로 시작한다.
- 주요 판단: 후보 5개를 유지하고, 실패 사례가 명백한 오답을 비활성화하며, 성공 사례의 공통 원칙과 현재 관찰로 남은 두 후보 중 하나를 고른다.
- 지원 경계: 권나래는 현장 재구성, 윤서하는 강제 반응, 오현은 공식 대조와 오답 하나 배제, 강이준은 안전선 확보, 한유리는 기억 잔향을 담당한다. 주인공 본인의 유형은 자동 지원으로 발동하지 않으며 지원 뒤에도 후보를 최소 둘 남긴다.
- 화면: 저승역 조사는 장소/지문·선택지/페이지형 매뉴얼 3열, 최종 검증은 비교 근거/보드/현장 반응 3열을 사용한다.
- 안정화 판단: 전조의 중립 질문 뒤 가설을 고르고, 확보한 근거를 선택한 다음 대응한다. 대응 카드에는 정답·가설 일치·최고 요원 수치를 미리 표시하지 않는다.
- 기록 승격: 현장 대응 성공은 안정화 판정이고 공식 매뉴얼 승격은 지식 검증이다. 올바른 대응이라도 가설 또는 모든 지지 근거가 없으면 후보로 남는다. 오대응은 위험 사례로 누적되며 이후 공식 규칙이 생겨도 삭제하지 않는다.
- 저장: 3×3은 내부 조작 학습이며 결과를 저장하지 않는다. 4×4 완료 때만 `completed`와 미니게임 결과를 한 번 저장한다. `anomaly_manual_records`는 `mvp-039`의 선택 필드로 저장한다.
...
| 임시 3인 편성 | 오현의 `balanced` 성향을 기존 요원 데이터에 맞춰 허용 | 구현 확인 |
| 위험도 상승 | 기본 1건, 확산이 높을 때 최대 2건 순환 상승 | 구현 확인 |
| 폭주 | 즉시 게임 오버가 아니라 긴급 조사·포기 판단 | 확정 설계 |
| 일상 이해도 | 최초 +2, 후속 +1, 하루 1회, 사건별 최대 +10 | 확정 설계 |
| 일상 카드 | `lead`·미해결 사건의 HQ 준비 화면에서만 열림. 완료 뒤 같은 준비 화면으로 복귀 | 구현 확인 |

## 6. 사건 상태와 콘텐츠

| 사건 | 현재 상태 | 콘텐츠 범위 | 다음 작업 |
|---|---|---|---|
| 저승역 | 구현 확인 | 목적지 공백 중심의 통합 현장 노드, 3×3 학습·4×4 안전 노선 검증, 회수·매뉴얼 초안 | 1280×720 실창 QA |
| 비 오는 골목의 빨간 우산 | 구현 확인 | 3단서, 3힌트, 통합 현장 노드, 빗소리/회피 판정, 회수·보고서 | 전체 플레이 문구·밸런스 QA |
| 폐주파수 방송국 | 구현 확인 | 위험도 60 발견 후 노출, 3단서·3회수 패턴·타이밍 판정·보고서·DB | 전용 아트·밸런스 검증. 현재 역 에셋을 임시 재사용 |
| 일상 에피소드 | 구현 확인 | 강이준–저승역 `막차 안내판의 빈칸`, 권나래–빨간 우산 `젖은 우산 보관함`, 오현–폐주파수 `무음이 남은 편집실`; 각 2선택·즉시 결과·DB 기록 | 플레이테스트로 총수와 요원 조합별 최소 분량 결정 |

일상 에피소드는 건너뛰어도 필수 단서, 조사, 회수, 엔딩 진행에 영향을 주지 않는다. 최초 완료 보상은 기존 사건별 일상 이해도 +2와 하루 1회·사건별 최대 +10 상한만 사용하며, 일자·반일·위험도·조사 편성·세력 의뢰는 바꾸지 않는다. 폐주파수 카드는 Day 8 자연 발견 뒤에만 열린다.
...
- 저승역: 공식 기록과 방송 원본을 비교해 안전 노선을 재구성하는 노선 복원 검증.
...
- `다음 전조 관측` 입력은 두지 않는다. 행동 결과가 남은 상태에서 다음 전조와 새 선택지가 즉시 열린다.
- 대응 후보는 기본 화면의 동일 폭 가로 카드이며 소비품은 별도 보조 행에 둔다.
...
|---|---|---|---|---|
| 전조 | 현재 전조와 중립 질문을 읽는다 | 전조, 질문, 관련 확보 기록 | 비교할 규칙 이해 | 자동 예측이 정답 행동을 설명하지 않는다 |
| 가설 | 현상을 설명할 가설 하나를 선택 | 가설 문장만 표시 | 검증할 주장 확정 | 근거·반증·정답 대응을 툴팁으로 누설하지 않는다 |
| 근거 | 확보 기록을 하나 이상 선택 | 수집 여부, 기록 제목·내용 | 가설의 지지·반증 범위 확정 | 미수집 기록은 선택할 수 없다 |
| 대응 | 상황형 행동 하나를 선택 | 행동 의도와 실행 보조 역할 | 안정화, 피해, 후보·공식·위험 사례 | 반사 신경이나 수치보다 근거 해석이 우선한다 |
...
![권나래 전신 아트](../assets/characters/mvp043/agents/kwon_narae/full_body.png)
...
| 윤서하 | 강제 반응·현장 돌파 | 기존 돌파형 수치 틀 | 구현 확인 |
| 한유리 | 기억 잔향·변조 조건 분석 | 분석·교감형 수치 틀 | 구현 확인 |

권나래는 고정 주인공이다. 준비 화면은 권나래 전신과 현재 반일 상태를 중앙에 두고, 나머지 네 요원 중 최대 둘을 서포트로 편성한다. 박도윤·이세린·레이먼드 케인·카밀라 바르가스는 외부 접점 카드로만 노출하며 고용·비용·현장 편성은 제공하지 않는다.
...
플레이어 노출명 `기록관 아카 · 괴이 기록국 관제 AI`는 전자기기 데스크와 현장 단말기를 통해 참여하는 디지털 유령 마스코트다. 내부 `로그` ID·튜토리얼 키·저장 키는 호환을 위해 유지한다.
...
- 현장에서는 `아카 안내` 버튼으로 관련 사례 위치와 비교 항목만 제시한다.
...
| 저장 버전 | `mvp-039` |
| 화면 버전 | `Ver 4.2` |
| 캠페인 이관 | `mvp-038` 저장은 누락된 일상 상태를 빈값으로 보완 |
| 핵심 보존 | 사건·단서·요원·장비·보고서·괴이 매뉴얼·세력·캠페인·의뢰 난수 상태·일상 완료 기록·진행 중 일상 |

저장, 진행, 경제, 엔딩, 에피소드 의미는 고위험 변경이다. `anomaly_manual_records`는 공식 규칙·후보·위험 사례를 보존하기 위한 선택 필드이며, 필드가 없는 기존 `mvp-039` 저장은 빈 기록으로 이관한다. UI 표현은 이 상태를 읽으며 별도 정답 소유자가 되지 않는다.
...
- 관찰: [The Case of the Golden Idol](https://store.steampowered.com/app/1677770/The_Case_of_the_Golden_Idol/)은 관찰 근거를 바탕으로 플레이어가 스스로 이론을 세우는 탐정 게임을 약속한다. [Return of the Obra Dinn](https://store.steampowered.com/app/653530/Return_of_the_Obra_Dinn/)의 기록 재검토 원칙도 기존 참고 기준과 일치한다.
- 개선점: 방송의 호출문·무음·피해자 목소리를 같은 정보로 뭉치지 않으면 회수 정답이 찍기로 읽힌다.
- 적용: 세 단서를 각각 “응답하면 이름이 복사됨 / 무음에서만 방송이 듣지 못함 / 실제 목소리는 뒤에서 앞에 덧씌워짐”으로 분리하고, 세 회수 전조와 오대응 이유에 재사용했다.
- 제외: 단어 조합 퍼즐, 기록장 UI 복제, 별도 추리 시스템·저장 필드는 추가하지 않았다.

### 텍스트 노벨 벤치마킹과 MVP-041 반영

확인일: 2026-07-14. 1차 기준은 활협전·화산의 딸·서울 2033·The Winter Tower이며, 상세 근거와 DeepSeek 읽기 전용 반대검토는 `docs/benchmarks/TEXT_NOVEL_BENCHMARK_2026-07-14.md`를 따른다. 평점·리뷰 수는 확인 시점 표본이고, 순위가 아니라 선택·일정·반복 피로·실패 피드백을 판단하는 보조 근거다.

| 사례 | 관찰 | 기록국 적용 또는 제외 | 상태 |
|---|---|---|---|
| 활협전 | 선택·행동이 성장·관계·서사를 바꾸지만 가혹한 운명과 처벌도 핵심 톤이다. | 결과에서 요원 기여·판단이 남게 한다. 영구 처벌·과도한 RNG·대규모 관계 육성은 제외한다. | 구현 확인 / 제외 |
| 화산의 딸 | 일정 선택과 성장 결과가 반복 동기를 만들며, 엔딩 수집 피로는 완화 설계가 필요하다. | 오전·오후 맥락과 선택 비용을 읽게 한다. 대규모 스탯·엔딩 수집은 제외한다. | 구현 확인 / 제외 |
| 서울 2033 | 짧은 독립 사건과 즉시 결과가 반복을 이끌지만 로그라이크 압박은 피로도 만든다. | HQ 일상 3장은 독립 카드·즉시 결과·사건 기록으로 구현했다. 영구 사망·광고·과금·자원 압박은 제외한다. | 구현 확인 / 제외 |
| The Winter Tower | 결말·수집은 완주 동기가 되지만 작은 표본은 구조 참고로만 쓴다. | 보고서·DB의 완료 기록은 기록국 고유 정체성으로 유지한다. 로맨스·진엔딩·일러스트 수집 중심 구조는 제외한다. | 구현 확인 / 제외 |
| 보조 참고작 | Slay the Princess, Citizen Sleeper, Roadwarden, Sir Brante는 기존 추리·시간·실패 표현의 보조 사례다. | 1차 텍스트 노벨 채택 판단에는 사용하지 않는다. | 보조 참고 |

텍스트 노벨 UX 계약은 `선택 전의 반일·근거 맥락 → 선택 직후의 서사 결과 → 사건 종료 뒤의 근거·요원 기여·다음 판단`이다. 보고서·DB는 네 참고작의 공통 규칙이 아니라 기존 기록국 정체성임을 명시하며, 실패 학습은 영구 차단 대신 공정 추리를 지키는 프로젝트 선택으로 유지한다.

### MVP-042 일상 에피소드 반영

기존 서울 2033 근거를 유지했으며, 이미 확정한 독립 카드 구조를 구현·검증하는 작업이어서 신규 광범위 조사는 하지 않았다. HQ 준비 화면에는 관련 사건이 `lead`이고 미해결일 때만 카드가 보이며, 각 카드는 두 선택지와 즉시 결과·요원 반응·DB 기록으로 합류한다. 최초 완료 보상은 기존 일상 이해도 +2만 사용한다.

| 확인 항목 | 구현 | 상태 |
|---|---|---|
| 비소모 경계 | 반일 일정·현장 조사·위험도·조사 편성·세력 의뢰를 변경하지 않는다. | 구현 확인 |
| 노출 | 저승역·빨간 우산은 `lead`·미해결, 폐주파수는 Day 8 자연 발견 뒤 `lead`·미해결일 때만 표시한다. | 구현 확인 |
| 기록 | 카드 ID·담당 요원·관련 사건·선택·결과 요약·보상·일자·반일을 저장하고 DB에 표시한다. | 구현 확인 |
| 호환성 | `mvp-038` 저장은 새 일상 필드를 빈값으로 보완한다. | 구현 확인 |

DeepSeek 읽기 전용 반대검토의 “일상 상태·기록은 사건 진행과 분리” 관찰은 채택했다. 반일 일정이나 경제를 일상에 추가하는 제안은 이 프로젝트의 일정 비소모 계약과 충돌하므로 제외했다. 자동 회귀는 `tests/test_mvp042_daily_episodes.gd`와 통합 러너를 따른다.
...
| 두 사건 수동 검증 | 성공·실패 후 다섯 질문에 답할 수 있음 | 네 경로의 저장 왕복·보고서·DB와 두 해상도를 검증했으며 현재 회귀는 `tests/test_mvp039_manual_ux_validation.gd`가 보호한다. | 구현 확인 |
...
| 범위 | 상태 | 현재 실행 근거 |
|---|---|---|
| MVP-001~043 | 구현 확인 | `tests/run_godot_regression.sh`가 기존 스크립트·씬 테스트 진입점을 격리 실행 |
| 세 사건 캠페인·저장·보고서·DB | 구현 확인 | `tests/test_three_case_campaign_manual_qa.gd`, `tests/test_two_case_campaign_manual_qa.gd` |
| CORE-VALIDATION-001 1A | 구현 확인 | `tests/test_core_validation_contract.py`의 오버레이·ID·수치·저장 계약 |
| CORE-VALIDATION-001 1B | 구현 확인 | `tests/core_validation_guided_flow_test.gd`의 가설·근거·대응·입력·폴백 |
| CORE-VALIDATION-001 1C | 구현 확인 | `tests/core_validation_manual_promotion_test.gd`의 후보·공식 규칙·위험 사례·저장 왕복 |
| 활성 문서 최신성 | 완료 기준 | `tests/test_active_document_references.py`의 기준선·경로·구형 참조 감사 |
| 외부 GPT 문구 패스 | 선택 계획 | 입력 ZIP 준비, 결과 미적용 |
| 180~220분 측정 | 계획 | 전체 콘텐츠 완성 후 실측 필요 |

## 19. 로드맵

### 현재 기준선: GDD v1.1 / MVP-043 + CORE-VALIDATION-001 / Ver 4.2

- 살아 있는 기획서와 DOCX 동기화 체계를 프로젝트 기준으로 고정한다.
- 현재 구현과 계획 콘텐츠의 경계를 문서에서 명확히 유지한다.
- 세 사건의 캠페인 계약은 Day 8 자연 발견, 오대응 학습, 보고서·DB·저장 왕복, 10일 마감까지 QA로 확인했다.
- HQ 일상 3장은 선택형 보조 기록으로만 동작하며, 캠페인 반일과 핵심 추리에는 영향을 주지 않는다.
- 저승역 첫 사건은 권나래 주인공과 오현·강이준 서포트, 사례 비교형 5개 후보, 3×3 학습 뒤 4×4 최종 검증으로 진행한다. 안정화에서는 가설·근거·대응을 분리하고 공식 규칙·후보·위험 사례를 장기 기록한다.
...
| P1 | 아카·요원 문구 패스 | 미확보 정보 누설·중복·톤 불일치 검수 |
...
| 버전 | 날짜 | 기준 | 변경 |
|---|---|---|---|
| v1.1 | 2026-07-22 | MVP-043 + CORE-VALIDATION-001 / Ver 4.2 / mvp-039 | 저승역 가설·근거·대응 단계형 안정화, 공식 규칙·검증 대기 후보·위험 사례 선택 필드 저장, 결과·DB 동적 기록, 활성 문서 참조 감사를 통합 |
| v1.0 | 2026-07-16 | MVP-043 / Ver 4.1 / mvp-039 | 중앙 조사 진행·팀 상태·기록관 아카, 자동 전조 회수, 권나래 고정 주인공과 초기 5인 저장 이관, 권나래 일정 대시보드, 외부 4인 접점, 캐릭터 아트 28종을 통합 |
| v0.9 | 2026-07-15 | MVP-043 / Ver 4.1 / mvp-039 | 저승역 3열 안전 UI, 페이지형 매뉴얼·5개 후보, 주인공 1명·서포트 0~2명, 주인공 단일 일정, 3×3 비저장·4×4 단일 저장, 결과·DB 연결을 추가 |
| v0.8 | 2026-07-14 | MVP-042 / Ver 4.1 / mvp-039 | 일정 비소모 HQ 일상 에피소드 3장, 카드·진행 중 상태·완료 기록 저장, `mvp-038` 빈값 이관, DB 기록과 두 해상도 UI QA를 추가 |
| v0.7 | 2026-07-14 | MVP-041 / Ver 4.0 / mvp-038 | 텍스트 노벨 1차 기준을 활협전·화산의 딸·서울 2033·The Winter Tower로 교체하고, 세 사건 자연 캠페인·오대응·보고서·DB·저장·해상도 QA를 기록 |
| v0.6 | 2026-07-14 | MVP-040 / Ver 4.0 / mvp-038 | 폐주파수 방송국의 발견 후 노출, 3단서·3회수 패턴·기존 판정·보고서·DB·저장 연결과 콘텐츠 벤치마킹 기록 추가 |
| v0.5 | 2026-07-14 | MVP-038 / Ver 3.8 / mvp-038 | 오현 임시 3인 편성 허용과 두 구현 사건의 전체 캠페인·저장·의뢰·10일 QA 기록 추가 |
| v0.4 | 2026-07-14 | MVP-038 / Ver 3.8 / mvp-038 | MVP-039 네 경로 UX·저장·보고서·DB·해상도 검증 기록과 다음 전체 캠페인 QA 기준 추가 |
