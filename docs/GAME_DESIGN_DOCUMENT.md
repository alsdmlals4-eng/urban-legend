...
> MVP-043 반영: 신규 캠페인은 저승역 오프닝과 오현·권나래·강이준의 고정 투입으로 시작한다. 조사 끝의 단일 노선 복원은 3×3 조작 학습 뒤 4×4 안전 노선 검증으로 이어지며, 마지막 기록은 회수와 괴이 매뉴얼 초안의 근거가 된다.
...
|---|---|
| 문서 버전 | v0.9 |
| 문서 상태 | MVP-043 저승역 3열 조사·매뉴얼 추리·주인공/서포트·3×3/4×4 검증 통합 |
| 기준 게임 버전 | Ver 4.1 |
...
- 저장 호환: `selected_agent_ids[0]`의 첫 유효 요원이 주인공이며 나머지 최대 두 명이 서포트다. 새 저장 필드나 마이그레이션은 없다.
- 일정: 반일 일정은 주인공에게만 배정·소비한다. 서포트는 독립 일정표를 갖지 않는다.
- 저승역 튜토리얼: 오현 주인공, 권나래 분석 서포트, 강이준 보호 서포트로 시작한다.
- 주요 판단: 후보 5개를 유지하고, 실패 사례가 명백한 오답을 비활성화하며, 성공 사례의 공통 원칙과 현재 관찰로 남은 두 후보 중 하나를 고른다.
- 지원 경계: 권나래는 첫 진입에 오답 최대 하나만 제외하고, 강이준은 첫 위험 결과의 피해만 완화한다. 주인공 본인의 유형은 자동 지원으로 발동하지 않는다.
...
| MVP-043 저승역 UI·주인공·괴이 매뉴얼 | 구현 확인 | [`MVP043_UI_PROTAGONIST_VALIDATION.md`](qa/MVP043_UI_PROTAGONIST_VALIDATION.md)의 3열 안전 프레임, 사례 기반 5개 후보, 주인공·서포트 저장 호환, 3×3 비저장·4×4 단일 저장, 결과·DB 회귀 |
| 외부 GPT 문구 패스 | 선택 계획 | 입력 ZIP 준비, 결과 미적용 |
| 180~220분 측정 | 계획 | 전체 콘텐츠 완성 후 실측 필요 |

## 19. 로드맵

### 현재 기준선: GDD v0.9 / MVP-043 저승역 UI·주인공·괴이 매뉴얼 교정
...
- 저승역 첫 사건은 오현 주인공과 권나래·강이준 서포트, 사례 비교형 5개 후보, 3×3 학습 뒤 4×4 최종 검증으로 진행한다.
