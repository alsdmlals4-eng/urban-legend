...
|---|---|
| 문서 버전 | v0.7 |
| 문서 상태 | MVP-041 기준 상세 통합 설계 + 세 사건 텍스트 노벨 캠페인 QA 완료 |
| 기준 게임 버전 | Ver 4.0 |
| 저장 스키마 | mvp-038 |
| 기준일 | 2026-07-14 |
...
| 폐주파수 방송국 | 구현 확인 | 위험도 60 발견 후 노출, 3단서·3회수 패턴·타이밍 판정·보고서·DB | 세 사건 전체 캠페인 수동 QA와 전용 아트·밸런스 검증. 현재 역 에셋을 임시 재사용 |
...
| 서울 2033 | 짧은 독립 사건과 즉시 결과가 반복을 이끌지만 로그라이크 압박은 피로도 만든다. | P1 일상은 독립 사건 카드·즉시 결과·사건 기록으로 설계한다. 영구 사망·광고·과금·자원 압박은 제외한다. | 계획 |
...
| MVP-041 세 사건 텍스트 노벨 캠페인 QA | 구현 확인 | [`MVP041_THREE_CASE_CAMPAIGN_MANUAL_QA.md`](qa/MVP041_THREE_CASE_CAMPAIGN_MANUAL_QA.md)의 Day 8 자연 발견, 세 사건 저장·보고서·DB, 오대응 학습, 10일 마감 테스트 135/135과 두 해상도 시각 QA |
| 외부 GPT 문구 패스 | 선택 계획 | 입력 ZIP 준비, 결과 미적용 |
| 180~220분 측정 | 계획 | 전체 콘텐츠 완성 후 실측 필요 |

## 19. 로드맵

### 현재 기준선: GDD v0.7 / MVP-041 세 사건 텍스트 노벨 캠페인 QA 완료
...
| 우선순위 | 목표 | 완료 기준 |
|---:|---|---|
| P1 | 일상 에피소드 최소 세트 | 서울 2033식 독립 사건 카드·즉시 결과·사건 기록을 적용하되, 선택형·비필수·사건별 이해도 상한을 지키는 요원 관계 콘텐츠 |
