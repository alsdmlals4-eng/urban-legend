...
- `D-2026-08-28-M04-BOUNDED-FORCED-DISPATCH-REACHABILITY`: M04는 범용 달력 없이 2주차 즉시 출동(`+0`) → 한 번 준비 뒤 3주차 지연 출동(`+15`, 귀가 기억 고정 강화) → 추가 준비 이득 없는 출동 보류 시 4주차 강제 출동(`+30`, modifier 없음)의 닫힌 flow를 사용한다. 위험이 어떤 runtime 피해 상태로 소비되는지는 후속 통합 구현 계약의 consumer mapping으로 남긴다.
...
- M04의 대표 고민은 `지금 출동해 보호를 우선할지, 한 번 준비해 피해자 위험 +15와 교환할지`다. 준비 뒤 출동을 더 보류하면 추가 이득 없이 4주차 `+30` 강제 출동이 된다. 지연 이득은 권나래의 능동형 `귀가 기억 고정` 공포 감소를 `-16 → -24`로 강화하는 것이며, 플레이어가 회수 중 직접 사용해야 한다. 결과는 Composite Result에서 추리·구출·회수와 분리된 출동 타이밍 인과로 설명한다.
