...
1. **GitHub Base 공용 규칙 원본 확인**
   ...
   - 특히 `docs/MVP_WORKFLOW_CHECKLIST.md`, `docs/AI_WORKFLOW_RULES.md`, `docs/BENCHMARKING_REFERENCE_GUIDE.md`를 우선 확인한다.
...
3. **프로젝트 전용 규칙 확인**
   ...
   - 현재 Issue / MVP 기준선
4. **벤치마킹 수행**
   - GPT는 작업마다 벤치마킹을 수행해 게임성, 디자인, UX, 아이디어, 시장성, 구현 방식 중 관련 항목을 확인한다.
...
이 규칙은 일반 작업 속도보다 우선한다. 사용자가 “바로 해줘”라고 해도 Base 확인, 벤치마킹, 좋은 프롬프트 변환을 생략하지 않는다.
...
11. 구현 범위 / 제외 범위 확정
12. 위험 분석 / 영향 파일 예상
13. GitHub Issue 작성 또는 수정
14. HTML 전체 흐름 대시보드 갱신 또는 로컬 HTML 파일 생성
15. Codex Goal 작성
16. Codex 구현 결과 검수
17. Godot 확인 항목 정리
18. 결과 보고 / 회고
19. Base 승격 후보 여부 판단
20. 필요한 경우 GitHub 공동 규칙 갱신
...
4. 프로젝트 전용 규칙 확인
5. 벤치마킹 수행 및 개선점/수정점 도출
6. 좋은 프롬프트 변환
7. 현재 MVP / Issue 기준선 확인
8. 구현 범위 / 제외 범위 확정
9. Issue 작성 또는 갱신
10. 전체 흐름 HTML 대시보드 갱신 또는 로컬 HTML 파일 생성
11. Codex Goal 작성
12. 로드맵 / 체크리스트 / 상태 감사 문서 갱신
13. Base 승격 후보 여부 판단
...
- Goal에는 HTML 대시보드 수정, 긴 벤치마킹 전문, 장문 세계관 설명을 넣지 않는다.
