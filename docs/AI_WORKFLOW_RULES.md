...
MVP-028부터 HTML 대시보드는 기본 산출물이 아니다. 사용자가 명시적으로 요청한 작업에서만 별도 범위와 검증 방법을 정해 작성한다. `docs/urban_legend_flow_dashboard.html`과 기존 다운로드 파일은 과거 기록이며 현재 구현 기준으로 사용하지 않는다.
