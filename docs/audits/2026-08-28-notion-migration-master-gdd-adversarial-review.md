...
- **Solution:** Trace each failure to the stale expected value; update only the authority assertions to match the current adapter, registry, and project decision; advance the adapter's trusted baseline to the immutable PR base while preserving the protected-path policy; reproduce the CI with a clean LF checkout; regenerate all six CI-pinned outputs; and commit the source plus views together. A Base upgrade is a separate future contract, not an incidental migration change.
- **Lesson:** Workspace-owner transitions require the metadata owner, its regression expectations, every generated operating view (including tracked agent routing files), byte-stable source encoding when a pinned generator calculates raw file hashes, and PR-base alignment whenever the canonical adapter itself changes. This is a project-operational lesson tied to an explicit user workflow choice, so `NO_BASE_PROMOTION`.

## Final disposition

No remaining blocking finding was found within this documentation/migration scope. PR #335's exact HEAD completed all 11 GitHub workflows successfully and squash-merged as `20e36777c15a2752da77fba3791846bff93b744f`; post-merge main readback confirmed the master GDD, migration receipt, and retained-reference registry. The only material product decision still open is the early-versus-regular numeric balance model. This review does not authorize production mutations.
