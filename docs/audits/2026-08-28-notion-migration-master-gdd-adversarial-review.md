...
- **Solution:** Trace each failure to the stale expected value; update only the authority assertions to match the current adapter, registry, and project decision; advance the adapter's trusted baseline to the immutable PR base while preserving the protected-path policy; normalize the canonical JSON to LF only while generating its CI-pinned dependent views; and commit the source plus views together. A Base upgrade is a separate future contract, not an incidental migration change.
- **Lesson:** Workspace-owner transitions require the metadata owner, its regression expectations, every generated operating view, byte-stable source encoding when a pinned generator calculates raw file hashes, and PR-base alignment whenever the canonical adapter itself changes. This is a project-operational lesson tied to an explicit user workflow choice, so `NO_BASE_PROMOTION`.
