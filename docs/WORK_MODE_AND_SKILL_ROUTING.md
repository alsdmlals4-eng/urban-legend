...
→ 필요한 최소 Base Skill과 프로젝트 분야 Skill 선택
...
- Base Skill은 Registry에 고정된 Base 커밋의 경로를 읽는다. 전체 `skills/`를 복사하거나 기본 로드하지 않는다.
...
| 기획 책임 원본 작성·수정 | `managing-design-documents: author/update/restructure` |
| 발행본 갱신 | 발행 정책 확인 뒤 `publish → validate` |
| 핵심 컨셉·벤치마크·플레이테스트 | `analyzing-and-refining-game-concepts`의 필요한 Skill Mode만 선택 |
| 구현·문서·데이터 변경 검증 | `reviewing-and-validating-project-changes`의 계약·정적·런타임·회귀 모드 |
