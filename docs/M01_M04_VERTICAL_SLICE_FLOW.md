...
- 그 이득은 `귀가 기억 고정 강화`다. M04 회수에서 플레이어가 기존 권나래 지원을 직접 사용할 때 공포 감소는 `-16 → -24`가 되며, 임계치 `+2`·안정화 변화 `0`·타 요원 지원은 그대로다.
- 한 번 준비하면 3주차 출동 docket으로 이동한다. 여기서 출동을 보류하면 추가 준비 이득 없이 즉시 4주차 `+30` 강제 출동 docket으로 escalation한다. 4주차에는 강화 modifier 없이 확인 후 M04가 시작된다.
