...
- 전역 cadence는 **10일 × 오전/오후 반일 슬롯**이며, 사건 하나를 한 cycle 안에 해결한다.
- Day 1~9 해결은 조기 해결, Day 10 해결은 정규 해결이다. Day 10을 지연·강제·벌점으로 표현하지 않는다.
- 조기 해결은 피해자를 더 일찍 보호하고, 정규 해결은 남은 반일을 준비에 쓰는 정상 선택이다. `2/3/4주`, 노출 `0/15/30`, 준비 tier `0/+4/+8`은 이전 결정의 `SUPERSEDED` 수치이며 새 날짜에 환산하지 않는다.
- `victim_route_memory_exposure`는 구현 계약이 별도 consumer와 수치 의미를 정할 때만 M04 Composite Result에서 추리·구출·회수와 분리해 설명한다. M01 온보딩에는 이 복잡도를 추가하지 않는다.

### M04 순차 후일담 결과 계약

- M04의 결과는 한 화면의 카드·점수·목록이 아니라 `피해자 → 잔향 → 귀가 기억 → 기록국`의 짧은 logical page sequence로 읽는다.
- 한 페이지에는 한 가지 인과와 1~3문장의 후일담만 둔다. 귀가 기억 페이지는 조기/정규·해결일·실제 권나래 지원 사용을 같은 준비 인과로만 다룬다.
