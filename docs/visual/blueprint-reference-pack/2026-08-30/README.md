...
| `HGB-UI-08` | `08-manual-deduction-dossier.png` | `SUPERSEDED_IDENTITY_REFERENCE`: 기록철형 레이아웃의 보존용 전임 시안. 기록 보조의 이름·귀여운 루메 정체성이 현재 사용자 지시와 맞지 않아 현행 기준으로 쓰지 않는다. | `b939079d39005b1351c1926a56a888b1386a253f743adf58ba783dc6e885efcc` |
| `HGB-AUX-08` | `08-aka-guide-candidate.png` | `SUPERSEDED_IDENTITY_REFERENCE`: 잘못된 아카 표기와 성숙한 체형의 전임 기록 보조 후보. 독립 제품 자산·런타임 소비자는 없다. | `06975d4d6655b7b777cb5fef205342a3a357050af9f3a3949c77fa7d23daceeb` |
| `HGB-UI-09` | `09-manual-deduction-dossier-lume.png` | `USER_APPROVED / BLUEPRINT_REFERENCE_ONLY`: CASE-01 저승역용 루메가 보조하는 16:9 기록철 매뉴얼. INDEX, 긴 추리문, 출처·순서가 남은 2열 후보를 한 화면에서 비교한다. | `2f79baeff877e9cb2fef8a641e0cec88213c1def1bba9a40f1998806b0f0abcd` |
| `HGB-AUX-09` | `09-lume-afterlife-station-guide-candidate.png` | `USER_APPROVED / BLUEPRINT_REFERENCE_ONLY`: 저승역 사례를 위해 맞춰 입은 귀여운 기록 보조 루메 초상. 독립 제품 자산·런타임 소비자는 없다. | `4d22f0eaeff23c9ecdcbc5bc40678fab4b7185a01c2572b650ab6e68771bccf4` |

`HGB-UI-07`은 정보 구조의 보존용 전임 시안이며, 사용자의 “첫 번째 기록철 화면처럼”
이라는 명시적 피드백으로 레이아웃 기준에서 제외했다. `HGB-UI-08`은 그 레이아웃을
교정했지만 기록 보조의 이름과 인물 비율이 맞지 않아 정체성 기준에서 제외했다.
현재 승인 화면은 `HGB-UI-09`다. 이 시안은 사용자 참고 화면의 **한 장짜리 기록철,
본문 중심 밀도, 좌측 INDEX, 우측 2열 후보, 하단 기록 보조**라는 화면 문법을
참고했지만, 문구·사건 내용·기관 표식·루메 초상은 이 프로젝트용으로 새로 구성했다.

`HGB-UI-09`와 `HGB-AUX-09`은 `USER_APPROVED / BLUEPRINT_REFERENCE_ONLY`이며
사람용 블루프린트에만 쓰인다. 둘 다 새 런타임 자산이 아니며, `ASSET_MANIFEST.yml`의
...
- 입력 역할: 사용자가 제공한 괴이 메뉴얼, 캐릭터 라인업, 회수 대응 화면은
  ...
  복사하지 않았다.
- 사용자 승인: 2026-08-30, 여섯 장면 묶음 전체 승인. 같은 날 `HGB-UI-09`와
  `HGB-AUX-09`도 사용자의 “좋아 승인” 선언으로 사람용 블루프린트 참고로 승인했다.
