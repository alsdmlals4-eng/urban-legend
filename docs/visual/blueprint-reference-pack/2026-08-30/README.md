...
| `HGB-UI-09` | `09-manual-deduction-dossier-lume.png` | `GENERATED_CANDIDATE / USER_DIRECTED_REVISION`: CASE-01 저승역용 루메가 보조하는 16:9 기록철 매뉴얼. INDEX, 긴 추리문, 출처·순서가 남은 2열 후보를 한 화면에서 비교한다. | `2f79baeff877e9cb2fef8a641e0cec88213c1def1bba9a40f1998806b0f0abcd` |
| `HGB-AUX-09` | `09-lume-afterlife-station-guide-candidate.png` | `GENERATED_CANDIDATE / BLUEPRINT_ONLY`: 저승역 사례를 위해 맞춰 입은 귀여운 기록 보조 루메 후보 초상. 독립 제품 자산·런타임 소비자는 없다. | `4d22f0eaeff23c9ecdcbc5bc40678fab4b7185a01c2572b650ab6e68771bccf4` |
...
현재 검수 대상은 `HGB-UI-09`다. 이 시안은 사용자 참고 화면의 **한 장짜리 기록철,
본문 중심 밀도, 좌측 INDEX, 우측 2열 후보, 하단 기록 보조**라는 화면 문법을
참고했지만, 문구·사건 내용·기관 표식·루메 초상은 이 프로젝트용으로 새로 구성했다.

`HGB-UI-09`와 `HGB-AUX-09`은 아직 `GENERATED_CANDIDATE`이며 사람용
...
- 입력 역할: 사용자가 제공한 괴이 메뉴얼, 캐릭터 라인업, 회수 대응 화면은
  ...
  복사하지 않았다.
- 사용자 승인: 2026-08-30, 여섯 장면 묶음 전체 승인. `HGB-UI-09`와
  `HGB-AUX-09`은 그 뒤의 사용자 지시 기반 후속 후보이며 아직 별도 시각 승인을
  받지 않았다.
