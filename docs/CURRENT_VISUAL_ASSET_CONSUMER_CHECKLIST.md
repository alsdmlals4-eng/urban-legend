...
|---|---|---|---|---|
| `IMG-M01-01` | Main Menu backdrop + Dialogue `ArtLayer/Background` + dialogue location preview | `assets/backgrounds/afterlife_entrance.png` | `USER_AUTONOMOUSLY_AUTHORIZED_VISUAL_CANDIDATE / PRODUCT_ASSET_PROMOTION_PENDING` | `M01_ENTRANCE_BACKGROUND_ADAPT_01`은 일반 플랫폼 복도·중앙 빈 표지판/기둥이 entry threshold와 compact preview를 약화해 승격하지 않았다. `M01_ENTRANCE_BACKGROUND_ADAPT_02_20260828`은 descending stair/paired rail/clock landmark를 되돌렸지만 candidate-only다. 현행 파일을 유지하며, Main Menu 전용 이미지는 만들지 않는다. |
...
| ID | Actual consumer | Current file | Current status | Delete test / note |
|---|---|---|---|---|
| `IMG-M04-01` | Dialogue background + main-menu current-case preview | `assets/backgrounds/red_alley_entrance.png` | `CANDIDATE_NOT_PROMOTED / REUSE_REVIEW` | Current file is strongly photoreal and its convenience-store-like striped sign conflicts with the soft-anime-noir lock. `M04_ENTRANCE_BACKGROUND_ADAPT_01_20260827` removes all brand/text cues and clarifies the red-umbrella opening read, but remains a candidate until actual Dialogue + **non-compact** current-case-preview comparison proves the style/readability improvement. `CurrentCasePreview` is hidden below 1500×850; thus 1280×720 validates Dialogue only, while 1920×1080 validates both consumers. |
...
| `IMG-M04-03` | Recovery `ArtLayer/Background` | `assets/backgrounds/red_recovery.png` | `USER_AUTONOMOUSLY_AUTHORIZED_VISUAL_CANDIDATE / PRODUCT_ASSET_PROMOTION_PENDING` | #295의 `M04_RECOVERY_BACKGROUND_ADAPT_01`은 중앙 여백은 개선했지만 사진적 표면이 잠긴 소프트 애니메 누아르에 미달했다. `M04_RECOVERY_BACKGROUND_ADAPT_02_20260828`은 레이어 분리와 ActionDock 여백을 유지한 보정 후보이나 candidate-only다. 현행 파일과 별도 B/C·D overlay는 유지한다. Hera live capture는 타 프로젝트 bridge port 점유로 `NOT_RUN`이다. |
...
| `IMG-M04-05` | Recovery `AnomalyVisual` D | `assets/anomalies/cutouts/red_umbrella_d_cutout.png` with full fallback | `REPLACE_REQUIRED / USER_AUTHORIZED_VISUAL_CANDIDATE / PRODUCT_ASSET_PROMOTION_PENDING` | `M04_ANOMALY_D_ADAPT_01` is an RGBA, single-umbrella candidate with alpha-zero tested corners. It removes the ordinary-civilian/multi-figure overlap from the review target, but does not replace the existing product path or prove runtime readability. |
