...
| `IMG-M04-04` | Recovery `AnomalyVisual` B/C | `assets/anomalies/cutouts/red_umbrella_b_cutout.png` with full fallback | `USER_AUTONOMOUSLY_AUTHORIZED_VISUAL_CANDIDATE / PRODUCT_ASSET_PROMOTION_PENDING` | Existing cutout is an identifiable ordinary figure and its full fallback carries scenery/another reflection subject, conflicting with the single `AnomalyVisual` owner. `M04_ANOMALY_BC_ADAPT_01_20260827` is a transparent B/C-only candidate; it has not replaced the canonical cutout or passed runtime/Human QA. |
