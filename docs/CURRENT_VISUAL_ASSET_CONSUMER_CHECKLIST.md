...
|---|---|---|---|---|
| `IMG-M01-01` | Main Menu backdrop + Dialogue `ArtLayer/Background` + dialogue location preview | `assets/backgrounds/afterlife_entrance.png` | `CANDIDATE_NOT_PROMOTED / REUSE_REVIEW` | `M01_ENTRANCE_BACKGROUND_ADAPT_01`은 일반 플랫폼 복도·중앙 빈 표지판/기둥이 entry threshold와 compact preview를 약화해 승격하지 않았다. 현행 파일을 유지하며, Main Menu 전용 이미지는 만들지 않는다. 새 후보는 descending threshold와 shared preview composition을 함께 충족해야 한다. |
...
| `IMG-M04-05` | Recovery `AnomalyVisual` D | `assets/anomalies/cutouts/red_umbrella_d_cutout.png` with full fallback | `REPLACE_REQUIRED / ALPHA_CANDIDATE_BLOCKED` | Current ARGB cutout is a back-facing ordinary civilian, while its RGB fallback is a multi-figure environment; neither conveys a D-risk escalation beyond B/C. Two bounded transparent-candidate attempts returned checkerboard RGB pixels and were rejected without repository/Notion storage. Preserve the existing product path until a genuine alpha-capable candidate route is available. |
