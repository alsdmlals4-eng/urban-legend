...
| 프로젝트 Skill | 실행 패키지 10개 / 통합 1개 / 전체 기본 로드 금지 |
| 현재 상태 | 비파괴 동기화 PR 검토 중 / 프로젝트 책임 원본 경로 유지 / 원격 자동 병합 금지 |

## 동기화 방식

Base의 공용 원본은 원격 고정 커밋으로 참조한다. 전체 Base 파일·Skill을 프로젝트에 복제하지 않는다. 자동 라우팅에 필요한 Base Registry 메타데이터만 `skills/SKILL_REGISTRY.json`에 고정 스냅샷으로 보존하며, source blob SHA로 원본을 식별한다.
...
- Base 13개 Skill 고정 라우팅 메타데이터와 프로젝트 10개 실행 Skill: `skills/SKILL_REGISTRY.json`
- 프로젝트 분야 실제 Skill 패키지: `skills/disciplines/<skill-id>/SKILL.md`
...
- Base 활성 Skill 13개는 ID뿐 아니라 trigger·비사용 조건·review trigger·지식 상태를 원본 Registry와 일치시켰다.
- 프로젝트 분야 Skill은 실제 `SKILL.md`가 없던 라우팅 라벨 상태를 제거하고 10개 실행 패키지로 구성했다.
- 프로젝트 trigger는 분야 간 완전 중복이 없도록 구체화했다.
- `urban-legend-integration-review`는 고유 입력·산출물·검증이 없어 다음으로 통합했다.
...
- 통합 전 ID는 `skills/LEGACY_SKILL_ALIASES.md`에서 호환한다.
- 대표 routing example을 Registry에 두고 자동검사로 주 Skill 최대 1개·지원 Skill 최대 3개를 확인한다.
...
- 자동 Registry·Skill 패키지·trigger·routing·경로 binding·발행 호환·보존·회귀 검증과 GitHub Actions
...
- Registry 항목만 읽고 Skill 실행 완료로 보고하지 않는다.
- 프로젝트 해결안은 먼저 프로젝트에서 검증하고 공용성 확인 뒤 Base 변경 제안으로 분리한다.
- 제안 PR과 Base 활성 구현 PR을 합치지 않는다.
- Base 기준을 바꾸면 이 문서, 운영 모델, Registry, 경로 어댑터, Skill 패키지 검사와 테스트의 커밋·blob 값을 함께 갱신한다.
...
검사는 Base trigger 원본 일치, 프로젝트 Skill 패키지 1:1, front matter, 실제 책임 원본, trigger 중복, routing example, 통합 alias, 경로 어댑터와 발행 호환을 확인한다.
