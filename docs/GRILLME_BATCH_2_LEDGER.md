...
- 지속 결과를 쓰는 사건마다 하나의 `정본 앵커` 지정
- 정본 앵커는 출동 준비 확정 직전
- 당시 보유 자원 범위에서 팀·장비·소비품·기관 지원 재구성 가능
- 이후 성장·장비·관계를 과거 정본에 소급 반입하지 않음
- 조사·구출·회수 전투 중간 지점으로 직접 캠페인 되감기 금지
- 캠페인 정본 변경 시 사건 전체를 다시 진행
- 이미 본 비상호작용 대사·연출과 읽은 텍스트는 빠른 넘김 지원
- 짧은 비정본 콘텐츠에는 정본 앵커를 만들지 않음
- 구현·사람 검증 미승인
...
접근성 완화 기능을 사용한 플레이가 일반 클리어·종합 랭크·S 랭크·업적에 어떤 영향을 받는지 결정한다.
