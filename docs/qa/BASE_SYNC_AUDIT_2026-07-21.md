...
| `docs/AI_SHARED_WORK_RULES.md` | `COMPATIBILITY_STUB` | 최신 운영 모델로 연결, 오래된 복제 규칙 선택 차단 |
| `docs/AI_WORKFLOW_RULES.md` | `COMPATIBILITY_STUB` + 프로젝트 확장 | 최신 라우팅을 연결하고 외부 위임 문서 경로 보존 |
...
| 영역 | 상태 | 증거 |
|---|---|---|
| Base 기준 커밋 일치 | 자동 검사 예정 | `tests/test_base_operating_sync.py` |
| 최신 13개 Skill 완전성 | 자동 검사 예정 | Registry exact-set 검사 |
| 구형 Skill ID 비활성화 | 자동 검사 예정 | Registry·alias 대조 |
| 기존 책임 원본 경로 보존 | 자동 검사 예정 | canonical path 존재 검사 |
| 프로젝트 불변 용어·보호 경로 보존 | 자동 검사 예정 | `AGENTS.md` 계약 검사 |
| 변경 파일 범위 | PR diff 검수 | 게임 코드·데이터·Scene·자산 변경 0건 요구 |
| Godot headless | `NOT_RUN` | 게임 런타임 파일 미변경; 병합 전 필요 시 별도 실행 |
| 수동 플레이·화면 QA | `NOT_RUN` | 플레이어 화면 변경 없음 |
| GitHub Actions | `PENDING` | `.github/workflows/validate-base-operating-sync.yml` |

## 병합 전 PR 체크

- [ ] PR #41~#43을 병합하지 않거나 본 비파괴 PR로 대체한다.
- [ ] 변경 파일에 `project.godot`, `scripts/**`, `data/**`, `scenes/**`, `assets/**`가 없다.
- [ ] 삭제·rename이 없다.
- [ ] 자동 검증이 통과한다.
- [ ] Base commit이 모든 진입점·Registry에서 일치한다.
- [ ] 최신 Base Skill 13개가 정확히 등록되고 구형 Foundation ID가 활성 목록에 없다.
- [ ] `AGENTS.md`의 프로젝트 불변 조건과 보호 경로가 유지된다.
- [ ] 기존 책임 원본 8개 경로가 그대로 존재한다.
- [ ] 미실행 Godot·수동 QA는 `NOT_RUN`으로 남긴다.
