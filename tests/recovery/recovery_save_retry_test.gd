...
func run() -> void:
	...
	var stage_failure := "--stage-failure" in OS.get_cmdline_user_args()
	var pending: String = state.get_save_file_path() + ".pending"
	if stage_failure and (FileAccess.file_exists(pending) or DirAccess.dir_exists_absolute(pending)):
		push_error("Stage failure test requires an unused pending path")
		quit(1)
		return
	...
	for status in ["control_failure", "approved_withdrawal", "core_recovered"]:
		...
		# Exercise router refusal or an owned stage obstacle in isolated user data.
		var mode: String = session.get("_mode")
		if stage_failure:
			check(DirAccess.make_dir_absolute(pending) == OK, "create owned disk failure obstacle")
		else:
		...
		if dialog != null:
			...
			if stage_failure:
				check(DirAccess.remove_absolute(pending) == OK, "remove owned disk failure obstacle")
			dialog.confirmed.emit()
			await frames()
			...
			session.set("_mode", mode)
			if stage_failure:
				check(DirAccess.remove_absolute(pending) == OK, "remove owned disk failure obstacle after failed test")
