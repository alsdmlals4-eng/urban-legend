...
Assert-Contains (Join-Path $root 'docs\IMAGE_ASSET_WORKFLOW.md') 'ASSET_MANIFEST.yml'
Assert-Contains (Join-Path $root 'docs\IMAGE_ASSET_WORKFLOW.md') 'LEGACY_MIGRATION_PENDING_NON_AUTHORITY'
Assert-Contains (Join-Path $root 'ASSET_MANIFEST.yml') 'asset_id: "M01-ANOMALY-D-001"'
Assert-Contains (Join-Path $root 'ASSET_MANIFEST.yml') 'replacement_requires_approval: true'
