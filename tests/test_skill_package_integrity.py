...
NO_LOSS = {
    ...
    "urban-legend-game-design": ["처치", "위험 사례", "balance-review"],
    "urban-legend-ux-ui-accessibility": ["1280×720", "동등한", "interaction-review"],
    "urban-legend-engineering": ["저장", "보호 경로", "compatibility-review"],
    "urban-legend-technical-art-pipeline": ["Manifest", "재생성", "pipeline-review"],
    "urban-legend-art": ["식별성", "의미 키", "asset-spec"],
    "urban-legend-audio": ["음소거", "폴백", "mix-review"],
    "urban-legend-qa": ["NOT_RUN", "release-gate", "원래 실패"],
    "urban-legend-production-pm": ["차단 PR", "rollback", "production-handoff"],
    "urban-legend-analytics-user-research": ["표본", "자기보고", "synthesis"],
}
...
def candidate(raw: str) -> Path | None:
    ...
    if Path(value).suffix.lower() not in LOCAL_SUFFIXES:
        return None
    if value.startswith(LOCAL_PREFIXES) or value in {
        "AGENTS.md", "README.md", "START_HERE.md", "MVP_ROADMAP.md", "TEST_CHECKLIST.md", "project.godot"
    }:
        return ROOT / value
    return None
...
class SkillPackageIntegrityTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.disciplines = self.registry["project_disciplines"]
    ...
    def test_base_index_snapshot_is_frozen_and_complete(self) -> None:
        ...
        for required in ("project-core", "adversarial-review", "skill-body-simplification", "runtime-failure"):
    ...
    def test_registry_is_lean_and_routes_to_base_index(self) -> None:
        ...
        self.assertEqual(self.registry["routing_policy"]["max_project_local_skills"], 1)

    def test_project_registry_and_packages_are_one_to_one(self) -> None:
        expected = {x["path"] for x in self.disciplines}
        actual = {p.relative_to(ROOT).as_posix() for p in (ROOT / "skills/disciplines").glob("*/SKILL.md")}
        self.assertEqual(actual, expected)
        self.assertEqual(len(self.disciplines), 10)
        self.assertEqual(len(self.disciplines), len({x["skill_id"] for x in self.disciplines}))
        self.assertEqual(len(self.local_skills), 1)
        self.assertEqual(self.local_skills[0]["skill_id"], "urban-legend-investigation-case-authoring")
        for item in self.local_skills:
    ...
    def test_local_case_authoring_contract_is_complete(self) -> None:
        item = self.local_skills[0]
        path = ROOT / item["path"]
        text = path.read_text(encoding="utf-8")
        meta = fields(text)
        self.assertEqual(meta.get("name"), item["skill_id"])
        ...
        self.assertIn(item["primary_discipline"], discipline_ids)
        self.assertTrue(set(item["supporting_disciplines"]) <= discipline_ids)
        self.assertTrue(set(item["support_skills"]) <= EXPECTED_BASE_IDS)
    ...
    def test_representative_routing_examples(self) -> None:
        ...
        for case in self.registry["routing_examples"]:
            ...
            self.assertLessEqual(len(local), 1, case)
            self.assertEqual(primary[0] if primary else None, case["expected_primary"], case)
            self.assertEqual(local[0] if local else None, case["expected_local"], case)
    ...
    def test_entrypoints_expose_core_index_coverage_and_shared_contract(self) -> None:
        ...
        combined = "\n".join((ROOT / p).read_text(encoding="utf-8") for p in entrypoints)
        for token in (
