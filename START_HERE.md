...
3. Base trigger가 맞는 지원 Skill만 최대 3개 고른다.
4. 프로젝트 Skill은 `PROJECT_DISCIPLINE_CONTRACT.md`와 해당 본문을 읽는다.
5. Base Skill은 `BASE_SKILL_INDEX.json`에서 선택하고 고정 커밋의 해당 `SKILL.md`·명시 reference만 읽는다.
6. 실행 뒤 이유·수행·증거·미검증을 보고한다.
