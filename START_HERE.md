...
→ 자동 선택된 프로젝트 분야 skills/disciplines/<skill-id>/SKILL.md 최대 1개
...
Registry 항목만 읽고 Skill을 실행했다고 보고하지 않는다. 프로젝트 분야 Skill은 Registry의 `path`에 있는 실제 `SKILL.md`를 읽고, Base Skill은 고정 커밋의 `base_path` 전문을 읽은 뒤 해당 mode를 수행한다.
...
- Base 활성 Skill: 13개, 고정 커밋 원격 전문 참조
- 프로젝트 분야 Skill: 실행 패키지를 가진 10개, `skills/disciplines/<skill-id>/SKILL.md`
- 통합된 프로젝트 Skill: `urban-legend-integration-review` → Base 통합검수·운영체계 `verify`
- 전체 Skill 기본 로드: 금지
- 주 책임 프로젝트 분야 Skill: 최대 1개
- Foundation·검증·발행·Handoff 지원 Skill: 최대 3개
