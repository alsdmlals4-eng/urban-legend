...
- `docs/GAME_DESIGN_DOCUMENT.md`는 게임 설계의 편집 원본이고 `docs/URBAN_LEGEND_GAME_DESIGN.docx`는 생성 배포본이다.
- 작업 시작 시 실제 구현, 최신 MVP, Issue/Goal, GDD·로드맵·README 불일치를 확인한다.
- 작업 종료 시 최신화한 문서, 벤치마킹 결론, 반영한 개선점, 후속 후보, 사용 스킬·도구 효율을 기록한다.
