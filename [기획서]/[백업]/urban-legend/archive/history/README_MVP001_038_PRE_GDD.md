...
현재 버전은 `Ver 3.8`이며, MVP 번호를 소수점으로 반영합니다.

`MVP-038`은 오전 계획·실행·결과 확인 뒤 오후를 계획하는 순차 반일 운영을 적용합니다. 조사는 반일을 소비하고 HQ 복귀 시 같은 작전을 중단 상태로 저장해 재개할 수 있습니다. 세력 의뢰는 3칸 게시판의 파견형·회수 행동형으로 확장되며 공용 1d100 판정과 등급별 보상을 사용합니다. 저장 스키마는 `mvp-038`이고 기존 `mvp-037` 캠페인 저장을 이관합니다.

`MVP-037`은 최대 10일의 비선형 데모 운영 코어를 시작합니다. 준비 화면에서 전 요원의 오전·오후 일정을 정하고, 두 기존 사건은 단서 상태로 시작하며 세 번째 사건은 미확인 상태로 저장됩니다. 하루가 끝날 때 미해결 괴이 위험도는 기본 1건, 확산이 높은 경우에만 2건까지 순환 상승합니다. 폭주는 게임 오버가 아니라 긴급 조사로 전환되며, 선택형 일상 콘텐츠의 사건 이해도 보너스는 사건별 최대 +10입니다. 저장 스키마는 `mvp-037`이고 기존 `mvp-035` 저장은 캠페인 기본값을 받아 불러옵니다.

`MVP-035`는 괴담기록국 AI `로그`를 메인·사건 준비·시장·통합 현장·회수·결과·DB에 연결합니다. 로그는 일반 화자처럼 초상과 대사로 참여하고, 평상·집중·경고 표정과 공통 3음 접속 시그니처로 상태를 알립니다. 첫 상세 안내와 재방문용 짧은 안내를 분리해 확인 상태를 저장하며, 저장 스키마는 `mvp-035`입니다.
...
메인 조사 화면은 배경과 상황 설명을 우선하고, 필요한 로그 안내와 편성 요원의 짧은 얼굴 초상 반응을 하단 선택 도크에 함께 표시합니다. 조사 지점과 방식은 34:66 두 열에서 동시에 비교하며, 회수 화면은 대표 요원과 괴이의 대치 장면 위에 얇은 상태 HUD와 액션 카드만 겹칩니다. 상세 단서와 학습 기록은 필요할 때 여는 서랍으로 이동합니다.
...
- [ ] 메인 메뉴의 버전 표기가 현재 기준 `Ver 3.4`로 표시된다.
