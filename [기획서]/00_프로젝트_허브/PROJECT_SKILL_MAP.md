# 프로젝트 스킬 지도

> 자동 생성 파생본입니다. 수동 편집하지 마세요.
> SKILL_REGISTRY.json SHA-256: `70d3cc8fd1e2f5dd9c35e25adf430388c3c251e47b21fc87a13626a01de98e26`

## 필수 분야

- 설정·내러티브
- 게임 디자인
- UX·UI·접근성
- 개발·엔지니어링
- 테크니컬 아트·콘텐츠 파이프라인
- 아트
- 사운드
- QA
- 프로덕션·PM
- 분석·유저리서치
- 통합검수

## 분야별 진입 스킬

| 분야 | 진입 스킬 |
|---|---|
| 설정·내러티브 | urban-legend-narrative |
| 게임 디자인 | urban-legend-game-design |
| UX·UI·접근성 | urban-legend-ux-ui-accessibility |
| 개발·엔지니어링 | urban-legend-engineering |
| 테크니컬 아트·콘텐츠 파이프라인 | urban-legend-technical-art-pipeline |
| 아트 | urban-legend-art |
| 사운드 | urban-legend-audio |
| QA | urban-legend-qa |
| 프로덕션·PM | urban-legend-production-pm |
| 분석·유저리서치 | urban-legend-analytics-user-research |
| 통합검수 | urban-legend-integration-review |

## 활성 스킬

| Skill ID | 계층 | 분야 | Trigger |
|---|---|---|---|
| urban-legend-narrative | discipline | 설정·내러티브 | narrative |
| urban-legend-game-design | discipline | 게임 디자인 | game-design |
| urban-legend-ux-ui-accessibility | discipline | UX·UI·접근성 | ux-ui-accessibility |
| urban-legend-engineering | discipline | 개발·엔지니어링 | engineering |
| urban-legend-technical-art-pipeline | discipline | 테크니컬 아트·콘텐츠 파이프라인 | technical-art-pipeline |
| urban-legend-art | discipline | 아트 | art |
| urban-legend-audio | discipline | 사운드 | audio |
| urban-legend-qa | discipline | QA | qa |
| urban-legend-production-pm | discipline | 프로덕션·PM | production-pm |
| urban-legend-analytics-user-research | discipline | 분석·유저리서치 | analytics-user-research |
| urban-legend-integration-review | discipline | 통합검수 | integration-review |
| routing-project-work-by-discipline | foundation | 프로젝트 전체 | routing |
| conducting-deep-requirement-interviews | foundation | 프로젝트 전체 | interview |
| migrating-existing-game-project-structure | foundation | 프로젝트 전체 | migration |
| maintaining-project-context-and-handoff | foundation | 프로젝트 전체 | handoff |
| evolving-project-discipline-skills | foundation | 프로젝트 전체 | skill-evolution |
| transforming-requests-into-prompts | foundation | 프로젝트 전체 | prompt |
| publishing-discipline-bibles | foundation | 프로젝트 전체 | publication |
| verifying-game-project-operating-system | foundation | 프로젝트 전체 | health-review |
