...
> MVP-043 반영: 권나래를 고정 주인공으로 삼고 오현·강이준을 기본 서포트로 투입한다. 초기 요원은 윤서하·한유리를 포함한 5명이며, 조사 끝의 3×3 학습과 4×4 검증은 자동 전조형 회수와 괴이 매뉴얼 기록으로 이어진다.
...
|---|---|
| 문서 버전 | v1.0 |
| 문서 상태 | MVP-043 현장 UI·자동 전조 회수·권나래 주인공·초기 5인·캐릭터 아트 통합 |
| 기준 게임 버전 | Ver 4.1 |
...
- 저장: 3×3은 내부 조작 학습이며 결과를 저장하지 않는다. 4×4 완료 때만 `completed`와 미니게임 결과를 한 번 저장한다.
...
| 핵심 보존 | 사건·단서·요원·장비·보고서·세력·캠페인·의뢰 난수 상태·일상 완료 기록·진행 중 일상 |
...
DeepSeek 읽기 전용 반대검토의 “일상 상태·기록은 사건 진행과 분리” 관찰은 채택했다. 반일 일정이나 경제를 일상에 추가하는 제안은 이 프로젝트의 일정 비소모 계약과 충돌하므로 제외했다. 상세 검증은 [`MVP042_DAILY_EPISODE_VALIDATION.md`](../../08_QA/등록_부록/MVP042_DAILY_EPISODE_VALIDATION.md)를 따른다.
...
| 두 사건 수동 검증 | 성공·실패 후 다섯 질문에 답할 수 있음 | 네 경로의 저장 왕복·보고서·DB와 두 해상도를 기록했다. 상세는 [`MVP039_MANUAL_UX_VALIDATION.md`](../../08_QA/등록_부록/MVP039_MANUAL_UX_VALIDATION.md)를 따른다. | 구현 확인 |
...
| MVP-039 | 완료 기준 | 기존 저장·사건 데이터를 재사용한 조사·회수·결과 판단 근거 UI와 네 경로 UX 검증 |
| 두 사건 자동·시각 QA | 구현 확인 | [`MVP039_MANUAL_UX_VALIDATION.md`](../../08_QA/등록_부록/MVP039_MANUAL_UX_VALIDATION.md)에 1280×720·1920×1080 검증 기록 |
| 두 사건 전체 캠페인 QA | 완료 기준 | [`TWO_CASE_CAMPAIGN_MANUAL_QA.md`](../../08_QA/등록_부록/TWO_CASE_CAMPAIGN_MANUAL_QA.md)에 3인·HQ 중단·저장·보고서·DB·의뢰·10일 검증 기록 |
| MVP-040 폐주파수 방송국 | 구현 확인 | [`MVP040_DEAD_FREQUENCY_SLICE.md`](../../08_QA/등록_부록/MVP040_DEAD_FREQUENCY_SLICE.md)의 발견 후 노출, 3단서·3회수 패턴·기존 타이밍 판정, 보고서·DB·저장·장면 경로 테스트 20/20 |
| MVP-041 세 사건 텍스트 노벨 캠페인 QA | 구현 확인 | [`MVP041_THREE_CASE_CAMPAIGN_MANUAL_QA.md`](../../08_QA/등록_부록/MVP041_THREE_CASE_CAMPAIGN_MANUAL_QA.md)의 Day 8 자연 발견, 세 사건 저장·보고서·DB, 오대응 학습, 10일 마감 테스트 136/136과 두 해상도 시각 QA |
| MVP-042 일정 비소모 일상 에피소드 3장 | 구현 확인 | [`MVP042_DAILY_EPISODE_VALIDATION.md`](../../08_QA/등록_부록/MVP042_DAILY_EPISODE_VALIDATION.md)의 카드 노출·저장 왕복·`mvp-038 → mvp-039` 이관·비소모 경계·두 해상도 UI QA |
| MVP-043 현장 UI·회수·5인·아트 | 구현 확인 | [`MVP043_UI_PROTAGONIST_VALIDATION.md`](../../08_QA/등록_부록/MVP043_UI_PROTAGONIST_VALIDATION.md)의 중앙 조사 진행, 팀 상태·아카 치환, 자동 전조 회수, 권나래 저장 이관, 초기 5인·외부 접점 4인, 캐릭터 아트 28종, 3×3 비저장·4×4 단일 저장 회귀 |
| 외부 GPT 문구 패스 | 선택 계획 | 입력 ZIP 준비, 결과 미적용 |
| 180~220분 측정 | 계획 | 전체 콘텐츠 완성 후 실측 필요 |

## 19. 로드맵

### 현재 기준선: GDD v1.0 / MVP-043 현장 UI·회수 루프·5인·캐릭터 아트 통합
...
- 저승역 첫 사건은 권나래 주인공과 오현·강이준 서포트, 사례 비교형 5개 후보, 3×3 학습 뒤 4×4 최종 검증과 자동 전조형 회수로 진행한다.
