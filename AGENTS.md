...
- 모든 작업은 시작 전에 GitHub `alsdmlals4-eng/Base`의 최신 공용 규칙을 먼저 확인한다. 그 다음 프로젝트 `AGENTS.md`, 프로젝트 문서, 최신 사용자 요청, 현재 Issue/Goal, 실제 파일을 확인한다.
- 사용자의 요청이나 Goal은 그대로 구현하지 않는다. 먼저 왜 필요한지, 해결하려는 문제, 플레이어 경험, 구현 범위, 제외 범위, 완료 기준, 검증 방법을 정리한 좋은 작업 프롬프트로 변환한 뒤 작업한다.
- Base 규칙과 프로젝트 규칙 또는 최신 사용자 요청이 충돌하면, 최신 사용자 요청을 최우선으로 하고 충돌 내용과 적용 이유를 짧게 설명한다.
...
- Goal이나 요청을 구현할 때 더 좋은 해결 방법, 오류 예방책, 또는 범위 안의 개선점이 발견되면 그대로 지나치지 않는다. 왜 더 나은지, 기존 방식과 무엇이 달라지는지, 사용자 경험·일정·유지보수에 어떤 영향이 있는지 먼저 설명하고, 요청 범위 안에서 안전하게 수정할 수 있는 방향을 함께 제안한다.
